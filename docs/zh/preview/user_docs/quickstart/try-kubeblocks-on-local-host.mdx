...
2. 点击左侧栏的仪表盘图标，页面上会显示两个监控面板。
   ![Dashboards](/img/docs/zh/try-kb-on-local-dashboard.png)
3. 点击 **General** -> **MySQL**，监控 MySQL 集群的状态。
   ![MySQL_panel](/img/docs/zh/try-kb-on-local-mysql-panel.png)
...
4. 在另一个终端选项卡中删除 Leader Pod，然后通过 NYAN CAT 页面查看对集群的影响。
   ...
   ![NYAN CAT](/img/docs/zh/try-kb-on-local-nyancat.png)
