...
1. 查看 ApeCloud MySQL 集群版。
   ...
   在 `Topology` 中查看 Leader 节点的名称。在这个例子中，Leader 节点为 `mycluster-mysql-1`。
    ...
    ![describe_cluster](/img/docs/zh/failure_simulation_describe_cluster.png)
2. 删除 Leader 节点 `mycluster-mysql-2`，模拟节点故障。
    ...
    ![delete_pod](/img/docs/zh/failure_simulation_delete_pod.png)
3. 执行 `kbcli cluster describe`，查看 pod 状态和集群连接。
    ...
    ***结果***
    ...
    ![describe_cluster_after](/img/docs/zh/failure_simulation_describe_cluster_after.png)

    ApeCloud MySQL 集群版可以在几秒钟恢复连接。

    ![connect_cluster_after](/img/docs/zh/failure_simulation_connect_cluster_after.png)
...
1. 查看 ApeCloud MySQL 集群版 pod 角色。在本示例中，Leader 节点为 `mycluster-1`。
    ...
    ![describe_pod](/img/docs/zh/api-ha-grep-role.png)
2. 删除 Leader 节点 `mycluster-mysql-1`，模拟节点故障。
    ...
    ![delete_pod](/img/docs/zh/api-ha-delete-leader-pod.png)
3. 检查 pod 状态和集群连接。
    ...
    ![describe_cluster_after](/img/docs/zh/api-ha-delete-leader-pod-after.png)
    ...
    ![connect_cluster_after](/img/docs/zh/api-ha-leader-pod-connect-check.png)
...
1. 查看 ApeCloud MySQL 集群版。
   ...
   在 `Topology` 中查看 Follower 节点的名称。在这个例子中，Follower 节点是 `mycluster-mysql-0` 和 `mycluster-mysql-2`。
    ...
    ![describe_cluster](/img/docs/zh/failure_simulation_describe_cluster_after.png)
...
2. 删除 Follower 节点 `mycluster-mysql-0`。
    ...
    ![delete_cluster](/img/docs/zh/failure_simulation_delete_follower_pod.png)
...
3. 查看集群状态，发现 Follower 节点处于 `Terminating` 状态。
    ...
    ![describe_cluster_follower](/img/docs/zh/failure_simulation_describe_cluster_follower.png)

4. 连接到 MySQL 集群版，发现单个 Follower 的异常不影响集群的读写操作。

   ![connect_cluster_follower](/img/docs/zh/failure_simulation_connect_cluster_follower.png)
...
1. 查看 pod 角色，如下示例中 follower 节点为 `mycluster-mysql-1` and `mycluster-mysql-2`。
    ...
    ![describe_cluster](/img/docs/zh/api-ha-grep-role-single-follower-pod.png)
2. 删除 follower 节点 `mycluster-mysql-1`。
    ...
    ![delete_follower_pod](/img/docs/zh/api-ha-single-follower-pod-delete.png)
3. 打开一个新的终端窗口，查看 pod 状态。可以发现 follower 节点 `mycluster-mysql-1` 处于 `Terminating` 状态。
    ...
    ![view_cluster_follower_status](/img/docs/zh/api-delete-single-follower-pod-status.png)

    再次查看 pod 角色。

    ![describe_cluster_follower](/img/docs/zh/api-ha-single-follower-pod-grep-role-after.png)
...
4. 连接集群，发现单个 Follower 节点异常不影响集群的读写操作。
    ...
    ![connect_cluster_follower](/img/docs/zh/api-ha-connect-single-follower-pod.png)
...
1. 查看 ApeCloud MySQL 集群版。
   ...
   在 `Topology` 中查看 Follower 节点的名称。在这个例子中，Follower 节点是 `mycluster-mysql-0` 和 `mycluster-mysql-2`。
    ...
    ![describe_cluster](/img/docs/zh/failure_simulation_describe_cluster_2.png)
2. 删除两个 Follower 节点。
    ...
    ![delete_two_pods](/img/docs/zh/failure_simulation_delete_two_pods.png)
3. 查看 MySQL 集群版状态，发现集群自动选举出了新的 Leader 节点。
    ...
    ![describe_two_pods](/img/docs/zh/failure_simulation_describe_two_pods.png)
4. 几秒钟后再次执行连接集群命令，数据库连接恢复正常。

    ![connect_two_pods](/img/docs/zh/failure_simulation_connect_two_pods.png)
...
1. 查看 Pod 状态。如下示例中，follower 节点为 `mycluster-mysql-1` 和 `mycluster-mysql-2`。
    ...
    ![describe_cluster](/img/docs/zh/api-ha-two-pods-grep-role.png)
2. 删除两个 follower 节点。
    ...
    ![delete_two_pods](/img/docs/zh/api-ha-two-pod-get-status.png)
3. 打开一个新的终端窗口，查看 pod 状态，发现两个 follower 节点 `mycluster-mysql-1` 和 `mycluster-mysql-2` 都处于 `Terminating` 状态。
    ...
    ![view_cluster_follower_status](/img/docs/zh/api-ha-two-pod-get-status.png)
    ...
    ![describe_cluster_follower](/img/docs/zh/api-ha-two-pods-grep-role-after.png)
...
4. 稍等几秒后，连接集群，发现集群中的 pod 在此正常运行。
    ...
    ![connect_two_pods](/img/docs/zh/api-ha-two-pods-connect-after.png)
...
1. 查看 ApeCloud MySQL 集群版。
   ...
   在 `Topology` 中查看节点名称。
    ...
    ![describe_cluster](/img/docs/zh/failure_simulation_describe_two_pods.png)
2. 删除所有节点。
    ...
    ![delete_three_pods](/img/docs/zh/failure_simulation_delete_three_pods.png)
3. 查看集群状态，发现节点角色发生改变。
    ...
    ![describe_three_clusters](/img/docs/zh/failure_simulation_describe_three_pods.png)
4. 几秒钟后再次连接数据库，数据库连接恢复正常。

    ![connect_three_clusters](/img/docs/zh/failure_simulation_connect_three_pods.png)
...
1. 查看节点角色。
    ...
    ![describe_cluster](/img/docs/zh/api-ha-all-pods-grep-role.png)
2. 删除所有节点。
    ...
    ![delete_three_pods](/img/docs/zh/api-ha-all-pods-delete.png)
3. 打开一个新的终端窗口，查看 pod 状态，发现所有 pod 均处于 `Terminating` 状态。
    ...
    ![describe_three_clusters](/img/docs/zh/api-ha-all-pods-get-status.png)
4. 再次查看节点角色，发现已选举产生新的 leader。
    ...
    ![describe_cluster_follower](/img/docs/zh/api-ha-all-pods-grep-role-after.png)
5. 稍等几秒后，连接集群，发现集群中的 pod 已恢复正常运行。
    ...
    ![connect_three_clusters](/img/docs/zh/api-ha-all-pods-connect-after.png)
