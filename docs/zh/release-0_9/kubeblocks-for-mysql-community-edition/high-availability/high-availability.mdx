...
1. 查看 MySQL 主备版 pod 角色。在本示例中，主节点为 `mycluster-mysql-0`。
    ...
    ![describe_pod](/img/docs/zh/api-mysql-ha-grep-role.png)
2. 删除主节点 `mycluster-mysql-0`，模拟节点故障。
    ...
    ![delete_pod](/img/docs/zh/api-mysql-ha-delete-primary-pod.png)
3. 检查 pod 状态和集群连接。
    ...
    ![describe_cluster_after](/img/docs/zh/api-mysql-ha-delete-primary-pod-after.png)
...
1. 查看集群信息。可在 `Topology` 中查看主节点的名称，在如下例子中，主节点为 `mycluster-mysql-0`。
    ...
    ![describe_cluster](/img/docs/zh/ha-mysql-describe-cluster.png)
2. 删除主节点 `mycluster-mysql-0`，模拟节点故障。
    ...
    ![delete_pod](/img/docs/zh/ha-mysql-delete-primary-pod.png)
3. 执行命令，查看 pod 状态和集群连接。
    ...
    ***Results***
    ...
    ![describe_cluster_after](/img/docs/zh/ha-mysql-primary-pod-describe-after.png)
...
1. 查看 pod 角色，如下示例中备节点为 `mycluster-mysql-0`。
    ...
    ![describe_cluster](/img/docs/zh/api-mysql-ha-grep-role-secondary-pod.png)
2. 删除备节点 `mycluster-mysql-0`。
    ...
    ![delete_secondary_pod](/img/docs/zh/api-ysql-ha-delete-secondary-pod.png)
3. 打开一个新的终端窗口，查看 pod 状态。可以发现备节点 `mycluster-mysql-0` 处于 `Terminating` 状态。
    ...
    ![view_cluster_secondary_status](/img/docs/zh/api-mysql-ha-secondary-pod-status.png)

    再次查看 pod 角色。

    ![describe_cluster_secondary](/img/docs/zh/api-mysql-ha-secondary-pod-grep-role-after.png)
...
1. 查看集群信息。可在 `Topology` 中查看备节点的名称，在如下例子中，备节点为 `mycluster-mysql-0`。
    ...
    ![describe_cluster](/img/docs/zh/ha-mysql-primary-pod-describe-after.png)
2. 删除备节点 `mycluster-mysql-0`。
    ...
    ![delete_secondary_pod](/img/docs/zh/ha-mysql-delete-secondary.png)
3. 查看集群状态，可在 `Component.Instance` 看到备节点处于终止状态。
    ...
    ![describe_cluster_secondary](/img/docs/zh/ha-mysql-delete-secondary-after.png)
...
1. 查看 pod 角色。
    ...
    ![describe_cluster](/img/docs/zh/api-mysql-ha-both-pods-grep-role.png)
2. 删除所有 pod。
    ...
    ![delete_both_pods](/img/docs/zh/api-mysql-ha-delete-both-pods.png)
3. 打开一个新的终端窗口，查看 pod 状态，发现所有 pod 均处于 `Terminating` 状态。
    ...
    ![describe_both_clusters](/img/docs/zh/api-mysql-ha-both-pods-status.png)
4. 再次查看节点角色，发现已选举产生新的主节点。
    ...
    ![describe_cluster](/img/docs/zh/api-mysql-ha-both-pods-grep-role-after.png)
...
1. 查看集群信息，在 `Topology` 中查看节点名称。
    ...
    ![describe_cluster](/img/docs/zh/ha-mysql-delete-secondary-after.png)
2. 删除所有节点。
    ...
    ![delete_three_pods](/img/docs/zh/ha-mysql-delete-both-pods.png)
3. 查看删除任务进展，发现所有节点都处于 pending 状态，几秒后恢复正常。
    ...
    ![describe_three_clusters](/img/docs/zh/ha-mysql-delete-both-pods-after.png)
