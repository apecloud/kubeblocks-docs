...
helm search repo kubeblocks-addons/orchestrator --versions
```
...
```bash
helm list -n kb-system| grep orchestrator
```

Example Output:
```bash
NAME                        	NAMESPACE  	REVISION	UPDATED                                	STATUS  	CHART                       	APP VERSION
kb-addon-orc                	kb-system  	1       	2026-02-09 11:12:32.286516 +0800 CST 	deployed	orchestrator-1.0.0          	3.2.6
```
...
Create an Orchestrator cluster first:
```bash
kubectl apply -f - <<EOF
...
When the Orchestrator cluster is ready, you can deploy the MySQL cluster:
...
:::note
If you encounter the Warning event message in MySQL Component, such as
```
Warning  Warning                   4m11s (x17 over 6m14s)  component-controller  config/script template has no template specified: mysql-replication-config
```
you can resolve this by annotating the corresponding `ComponentParameter` resource to trigger an on-demand reconcile.

For example:

```bash
kubectl annotate componentparameter example-mysql-cluster-mysql 'apps.kubeblocks.io/reconcile-now=' --overwrite -n demo
```

Replace `example-mysql-cluster-mysql` with the actual name of your `ComponentParameter` resource.
This annotation will prompt the KubeBlocks Operator to reconcile the resource and resolve the template issue.
:::
