...
Before restoring, ensure that there is a full backup is available. The restoration process will use this backup to create a new MongoDB cluster.
...
```bash
kubectl get backup mongodb-backup-datafile -n demo -ojson | jq -r '.metadata.annotations | ."kubeblocks.io/encrypted-system-accounts" | fromjson .mongodb | tojson |gsub("\""; "\\\"")'
```

Example Output:

```bash
{\"root\":\"8hgNxq8BV8IPjQktnOlLX0nw8lyrO59mJbtbiFP5D19d10qvzCJBB2mQ0FU=\"}
```
...
metadata:
  ...
  annotations:
    ...
    kubeblocks.io/restore-from-backup: '{"mongodb":{"encryptedSystemAccounts":"<ENCRYPTED-SYSTEM-ACCOUNTS>","name":"<FULL_BACKUP_NAME>","namespace":"demo","volumeRestorePolicy":"Parallel"}}'
spec:
