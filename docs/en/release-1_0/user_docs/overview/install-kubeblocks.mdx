...
Alternatively, browse all releases on [KubeBlocks Releases](https://github.com/apecloud/kubeblocks/releases/).
...
Run the following command to list stable releases:

```bash
kbcli kubeblocks list-versions
```

For pre-release versions (alpha/beta), add the `--devel` flag:
...
Install your chosen version using:
...
```bash
kbcli kubeblocks install --version=<VERSION> --create-namespace --namespace <NAMESPACE>
```
