...
spec:
  ...
  shardings:
  - name: shard  # shard is the name of the component
    shards: 3  # shards is the number of shards to create for the cluster
    template:
      name: redis
      componentDef: redis-cluster-7 # redis-cluster-7 is the name of the component definition for each shard
      ...
      - name: data
        spec:
          storageClassName: <STORAGE_CLASS> # STORAGE_CLASS is the name of the storage class to use for the cluster
          accessModes:
          - ReadWriteOnce
          resources:
            requests:
...
A Redis cluster needs a minimum of three master nodes to ensure high availability and prevent data inconsistency.  A production-ready Redis Cluster is typically recommended to have at least six nodes: three masters for sharding and failover consensus, and three replicas to act as backups for each master.
