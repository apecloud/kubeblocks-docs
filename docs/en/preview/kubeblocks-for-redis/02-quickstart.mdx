...
```bash
kubectl apply -f https://github.com/apecloud/kubeblocks-addons/blob/main/examples/redis/cluster.yaml
```
...
- Redis running with one Primary replica (read/write operations and one Secondary replica (read-only operations)
...
redis            redis-7-1.0.0-alpha.0            docker.io/redis/redis-stack-server:7.2.0-v10
                                                  docker.io/apecloud/agamotto:0.1.2-beta.1
                                                  docker.io/redis/redis-stack-server:7.2.0-v14
redis-sentinel   redis-sentinel-7-1.0.0-alpha.0   docker.io/redis/redis-stack-server:7.2.0-v14
...
<Tabs>
  ...
  ```bash
  kubectl patch cluster redis-replication -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "redis", "stop": true}, {"name": "redis-sentinel", "stop": true}]}}'
  ```
...
<Tabs>
  ...
  kubectl patch cluster redis-replication -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "redis", "stop": false}, {"name": "redis-sentinel", "stop": false}]}}'
  ```
  </TabItem>
</Tabs>
