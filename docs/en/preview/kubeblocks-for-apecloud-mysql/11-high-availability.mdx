...
2. Delete the leader pod `mycluster-mysql-1` to simulate a pod fault.
    ...
    ![delete_pod](/img/docs/en/api-ha-delete-leader-pod.png)
3. Check the status of the pods and RaftGroup Cluster connection.
    ...
    ![describe_cluster_after](/img/docs/en/api-ha-delete-leader-pod-after.png)
    ...
    ![connect_cluster_after](/img/docs/en/api-ha-leader-pod-connect-check.png)
...
1. View the ApeCloud MySQL RaftGroup information. View the leader pod name in `Topology`. In this example, the leader pod's name is `mycluster-mysql-2`.
    ...
    ![describe_cluster](/img/docs/en/failure_simulation_describe_cluster.png)
2. Delete the leader pod `mycluster-mysql-2` to simulate a pod fault.
    ...
    ![delete_pod](/img/docs/en/failure_simulation_delete_pod.png)
3. Run `kbcli cluster describe` and  connect to the cluster to check the status of the pods and RaftGroup connection.
    ...
    ***Results***
    ...
    ![describe_cluster_after](/img/docs/en/failure_simulation_describe_cluster_after.png)
    It shows that this ApeCloud MySQL RaftGroup can be connected within seconds.

    ![connect_cluster_after](/img/docs/en/failure_simulation_connect_cluster_after.png)
...
1. View the pod role again and in this example, the follower pods are `mycluster-mysql-1` and `mycluster-mysql-2`.
    ...
    ![describe_cluster](/img/docs/en/api-ha-grep-role-single-follower-pod.png)
2. Delete the follower pod `mycluster-mysql-1`.
    ...
    ![delete_follower_pod](/img/docs/en/api-ha-single-follower-pod-delete.png)
3. Open another terminal page and view the pod status. You can find the follower pod `mycluster-mysql-1` is `Terminating`.
    ...
    ![view_cluster_follower_status](/img/docs/en/api-delete-single-follower-pod-status.png)

    View the pod roles again.

    ![describe_cluster_follower](/img/docs/en/api-ha-single-follower-pod-grep-role-after.png)
...
4. Connect to this cluster and you can find this single follower exception doesn't affect the R/W of the cluster.
    ...
    ![connect_cluster_follower](/img/docs/en/api-ha-connect-single-follower-pod.png)
...
1. View the ApeCloud MySQL RaftGroup information and view the follower pod name in `Topology`. In this example, the follower pods are `mycluster-mysql-0` and m`mycluster-mysql-2`.
    ...
    ![describe_cluster](/img/docs/en/failure_simulation_describe_cluster_after.png)
2. Delete the follower pod `mycluster-mysql-0`.
    ...
    ![delete_follower_pod](/img/docs/en/failure_simulation_delete_follower_pod.png)
3. View the RaftGroup status and you can find the follower pod is being terminated.
    ...
    ![describe_cluster_follower](/img/docs/en/failure_simulation_describe_cluster_follower.png)
4. Connect to the RaftGroup and you can find this single follower exception doesn't affect the R/W of the cluster.

    ![connect_cluster_follower](/img/docs/en/failure_simulation_connect_cluster_follower.png)
...
1. View the pod role again. In this example, the follower pods are `mycluster-mysql-1` and `mycluster-mysql-2`.
    ...
    ![describe_cluster](/img/docs/en/api-ha-two-pods-grep-role.png)
2. Delete these two follower pods.
    ...
    ![delete_two_pods](/img/docs/en/api-ha-two-pod-get-status.png)
3. Open another terminal page and view the pod status. You can find the follower pods `mycluster-mysql-1` and `mycluster-mysql-2` is `Terminating`.
    ...
    ![view_cluster_follower_status](/img/docs/en/api-ha-two-pod-get-status.png)
    ...
    ![describe_cluster_follower](/img/docs/en/api-ha-two-pods-grep-role-after.png)
...
4. Connect to this cluster after a few seconds and you can find the pods in the RaftGroup Cluster work normally again.
    ...
    ![connect_two_pods](/img/docs/en/api-ha-two-pods-connect-after.png)
...
1. View the ApeCloud MySQL RaftGroup information and view the follower pod name in `Topology`. In this example, the follower pods are `mycluster-mysql-0` and `mycluster-mysql-2`.
    ...
    ![describe_cluster](/img/docs/en/failure_simulation_describe_cluster_2.png)
2. Delete these two follower pods.
    ...
    ![delete_two_pods](/img/docs/en/failure_simulation_delete_two_pods.png)
3. View the RaftGroup status and you can find the follower pods are pending and a new leader pod is selected.
    ...
    ![describe_two_pods](/img/docs/en/failure_simulation_describe_two_pods.png)
4. Connect to cluster again after a few seconds and you can find the pods in the RaftGroup work normally again.

    ![connect_two_pods](/img/docs/en/failure_simulation_connect_two_pods.png)
...
1. View the role of pods.
    ...
    ![describe_cluster](/img/docs/en/api-ha-all-pods-grep-role.png)
2. Delete all pods.
    ...
    ![delete_three_pods](/img/docs/en/api-ha-all-pods-delete.png)
3. Open another terminal page and view the pod status. You can find the pods are terminating.
    ...
    ![describe_three_clusters](/img/docs/en/api-ha-all-pods-get-status.png)
4. Run the command below to view the pod status. After a few seconds, you can find all pods are running again and a new leader is selected.
    ...
    ![describe_cluster_follower](/img/docs/en/api-ha-all-pods-grep-role-after.png)
5. Connect to the cluster and the connection works normally again.
    ...
    ![connect_three_clusters](/img/docs/en/api-ha-all-pods-connect-after.png)
...
1. Run the command below to view the ApeCloud MySQL RaftGroup information and view the pods' names in `Topology`.
    ...
    ![describe_cluster](/img/docs/en/failure_simulation_describe_two_pods.png)
2. Delete all pods.
    ...
    ![delete_three_pods](/img/docs/en/failure_simulation_delete_three_pods.png)
3. Run the command below to view the cluster and pod status. After a few seconds, you can find all pods are running again and a new leader is selected.
    ...
    ![describe_three_clusters](/img/docs/en/failure_simulation_describe_three_pods.png)
4. Connect to the cluster and the connection works normally again.

    ![connect_three_clusters](/img/docs/en/failure_simulation_connect_three_pods.png)
