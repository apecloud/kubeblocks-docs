...
```bash
kubectl apply -f https://github.com/apecloud/kubeblocks-addons/blob/main/examples/milvus/cluster-standalone.yaml
```
...
etcd        etcd-3-1.0.0-alpha.0              quay.io/coreos/etcd:v3.5.15
minio       milvus-minio-1.0.0-alpha.0        docker.io/minio/minio:RELEASE.2022-03-17T06-34-49Z
milvus      milvus-standalone-1.0.0-alpha.0   docker.io/milvusdb/milvus:v2.3.2
...
<Tabs>
  ...
  ```bash
  kubectl patch cluster milvus-standalone -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "milvus", "stop": true}, {"name": "etcd", "stop": true}, {"name": "minio", "stop": true}]}}'
  ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  kubectl patch cluster milvus-standalone -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "milvus", "stop": false}, {"name": "etcd", "stop": false}, {"name": "minio", "stop": false}]}}'
  ```
  </TabItem>
</Tabs>
