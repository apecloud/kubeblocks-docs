...
```bash
$ username=`kubectl get secret mysql-cluster-mysql-account-root -o jsonpath='{.data.username}' | base64 --decode`

$ passwd=`kubectl get secret mysql-cluster-mysql-account-root -o jsonpath='{.data.password}' | base64 --decode`
```
...
<Tabs>
  ...
  ```bash
  $ kubectl exec -ti -n default mysql-cluster-mysql-0 -- mysql -h mysql-cluster-mysql -u$username -p$passwd
  ```
  ...
  ```bash
  $ mysql -h 127.0.0.1 -P3306 -u$username -p$passwd
  ```
