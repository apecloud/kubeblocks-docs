...
<h3 id="apps.kubeblocks.io/v1.Cluster">
Cluster
...
Which, in conjunction with the <code>topology</code> field, determine:
</p>
<ul>
<li>
...
within <code>componentSpecs[&ast;].componentDef</code>.
</p>

<p>
If this field is not provided, each component must be explicitly defined in <code>componentSpecs[&ast;].componentDef</code>.
</p>
...
The ClusterDefinition may list multiple topologies under <code>clusterdefinition.spec.topologies[&ast;]</code>,
each tailored to different use cases or environments.
</p>

<p>
If <code>topology</code> is not specified, the Cluster will use the default topology defined in the ClusterDefinition.
</p>

<p>
Note: Once set during the Cluster creation, the <code>topology</code> field cannot be modified.
...
<ul>
<li>
<code>DoNotTerminate</code>: Prevents deletion of the Cluster. This policy ensures that all resources remain intact.
</li>
<li>
<code>Delete</code>: Deletes all runtime resources belong to the Cluster.
</li>
<li>
<code>WipeOut</code>: An aggressive policy that deletes all Cluster resources, including volume snapshots and
...
The <code>WipeOut</code> policy is particularly risky in production environments due to its irreversible nature.
</p>
...
<p>
Note: <code>shardings</code> and <code>componentSpecs</code> cannot both be empty; at least one must be defined to configure a Cluster.
</p>
...
through the <code>shards</code> field in ClusterSharding.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterDefinition">
ClusterDefinition
...
<h3 id="apps.kubeblocks.io/v1.Component">
Component
...
For example, a Redis Cluster can include Components like &lsquo;redis&rsquo;, &lsquo;sentinel&rsquo;, and potentially a proxy like &lsquo;twemproxy&rsquo;.
</p>
...
<p>
ServiceRefs with identical <code>serviceRef.name</code> in the same Cluster are considered the same.
</p>
...
It allows defining the CPU, memory requirements and limits for the Component&rsquo;s containers.
</p>
...
<ul>
<li>
<code>StrictInPlace</code> indicates that only allows in-place upgrades.
Any attempt to modify other fields will be rejected.
</li>
<li>
<code>PreferInPlace</code> indicates that we will first attempt an in-place upgrade of the Pod.
If that fails, it will fall back to the ReCreate, where pod will be recreated.
Default value is &ldquo;PreferInPlace&rdquo;
</li>
</ul>
...
<ol>
<li>
The associated Pod is stopped, and its PersistentVolumeClaim (PVC) is retained for potential
future reuse or data recovery, but it is no longer actively used.
</li>
<li>
The ordinal number assigned to this instance is preserved, ensuring it remains unique
...
<p>
Determines whether metrics exporter information is annotated on the Component&rsquo;s headless Service.
</p>
...
<li>
&ldquo;monitor.kubeblocks.io/scheme&rdquo;
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ComponentDefinition">
ComponentDefinition
...
<p>
Provides a brief and concise explanation of the Component&rsquo;s purpose, functionality, and any relevant details.
It serves as a quick reference for users to understand the Component&rsquo;s role and characteristics.
</p>
...
<p>
The <code>serviceKind</code> field allows users to quickly identify the type of Service provided by the Component
...
<ul>
<li>
&ldquo;MySQL&rdquo;: Indicates that the Component provides a MySQL database service.
</li>
<li>
&ldquo;PostgreSQL&rdquo;: Indicates that the Component offers a PostgreSQL database service.
</li>
<li>
&ldquo;Redis&rdquo;: Signifies that the Component functions as a Redis key-value store.
</li>
<li>
&ldquo;ETCD&rdquo;: Denotes that the Component serves as an ETCD distributed key-value store.
</li>
</ul>

<p>
The <code>serviceKind</code> value is case-insensitive, allowing for flexibility in specifying the protocol name.
</p>

<p>
When specifying the <code>serviceKind</code>, consider the following guidelines:
</p>
<ul>
<li>
...
<li>
Ensure that the <code>serviceKind</code> accurately represents the primary service type offered by the Component.
</li>
<li>
If the Component provides multiple services, choose the most prominent or commonly used protocol.
</li>
<li>
Limit the <code>serviceKind</code> to a maximum of 32 characters for conciseness and readability.
</li>
</ul>

<p>
Note: The <code>serviceKind</code> field is optional and can be left empty if the Component does not fit into a well-known
service category or if the protocol is not widely recognized. It is primarily used to convey information about
the Component&rsquo;s service type to users and facilitate discovery and integration.
</p>

<p>
The <code>serviceKind</code> field is immutable and cannot be updated.
</p>
...
It follows the syntax and semantics of the &ldquo;Semantic Versioning&rdquo; specification (<a href="http://semver.org/">http://semver.org/</a>).
</p>
...
<li>
&ldquo;3.0.0-alpha.1&rdquo;
</li>
<li>
&ldquo;4.5.2+build.1&rdquo;
</li>
</ul>

<p>
The <code>serviceVersion</code> field is immutable and cannot be updated.
</p>
...
<p>
Note: If a label key in the <code>labels</code> field conflicts with any system labels or user-specified labels,
...
<p>
Note: If an annotation key in the <code>annotations</code> field conflicts with any system annotations
...
They should be specified in the <code>cluster.spec.componentSpecs</code> (ClusterComponentSpec).
</p>
...
These instance-specific overrides can be specified in <code>cluster.spec.componentSpecs[&ast;].instances</code>.
</p>
...
<code>cluster.spec.componentSpecs[&ast;].volumeClaimTemplates</code> field to configure dynamic properties such as
...
<li>
Disk high watermark: Sets the high watermark for the volume&rsquo;s disk usage.
...
<p>
When <code>hostNetwork</code> option is enabled, the Pods share the host&rsquo;s network namespace and can directly access
...
If the query fails, it will fall back to the host&rsquo;s DNS settings.
</p>

<p>
If set, the DNS policy will be automatically set to &ldquo;ClusterFirstWithHostNet&rdquo;.
</p>
...
<p>
Defines additional Services to expose the Component&rsquo;s endpoints.
</p>

<p>
A default headless Service, named <code>&#123;cluster.name&#125;-&#123;component.name&#125;-headless</code>, is automatically created
for internal Cluster communication.
</p>

<p>
This field enables customization of additional Services to expose the Component&rsquo;s endpoints to
...
<code>componentDefinition.spec.vars[&ast;].valueFrom.serviceVarRef</code>.
</li>
<li>
For inter-Cluster access, reference Services use variables declared in
<code>componentDefinition.spec.vars[&ast;].valueFrom.serviceRefVarRef</code>,
and bind Services at Cluster creation time with <code>clusterComponentSpec.ServiceRef[&ast;].clusterServiceSelector</code>.
</li>
</ul>
...
<p>
This field specifies a list of templates that will be rendered into Component containers&rsquo; config files.
...
<p>
The <code>logConfigs</code> field is an optional list of LogConfig objects, where each object represents
...
<p>
An array of <code>SystemAccount</code> objects that define the system accounts needed
for the management operations of the Component.
</p>

<p>
Each <code>SystemAccount</code> includes:
</p>
<ul>
<li>
Account name.
...
<ul>
<li>
<strong>System Accounts</strong>: Created during Cluster setup by the KubeBlocks operator,
...
<li>
<strong>User Accounts</strong>: Managed by users or administrator.
...
<p>
The roles assigned to a replica can influence various aspects of the Component&rsquo;s behavior, such as:
</p>
<ul>
<li>
Service selection: The Component&rsquo;s exposed Services may target replicas based on their roles using <code>roleSelector</code>.
</li>
<li>
...
<p>
<code>minReadySeconds</code> is the minimum duration in seconds that a new Pod should remain in the ready
state without any of its containers crashing to be considered available.
This ensures the Pod&rsquo;s stability and readiness to serve requests.
</p>
...
<ul>
<li>
<code>Serial</code>: Updates replicas one at a time, ensuring minimal downtime by waiting for each replica to become ready
before updating the next.
</li>
<li>
<code>Parallel</code>: Updates all replicas simultaneously, optimizing for speed but potentially reducing availability
during the update.
</li>
<li>
<code>BestEffortParallel</code>: Updates replicas concurrently with a limit on simultaneous updates to ensure a minimum
...
<p>
This field is immutable and defaults to &lsquo;Serial&rsquo;.
</p>
...
<ul>
<li>
<code>OrderedReady</code>: Creates pods in increasing order (pod-0, then pod-1, etc). The controller waits until each pod
is ready before continuing. Pods are removed in reverse order when scaling down.
</li>
<li>
<code>Parallel</code>: Creates pods in parallel to match the desired scale without waiting. All pods are deleted at once
when scaling down.
</li>
</ul>
...
<p>
If not specified, the default behavior is the same as <code>podUpdatePolicy</code>.
</p>
...
<p>
The <code>policyRules</code> field is an array of <code>rbacv1.PolicyRule</code> objects that define the policy rules
...
<ul>
<li>
<code>postProvision</code>: Defines the hook to be executed after the creation of a Component,
...
<code>Immediately</code>, <code>RuntimeReady</code>, <code>ComponentReady</code>, and <code>ClusterReady</code>.
</li>
<li>
<code>preTerminate</code>: Defines the hook to be executed before terminating a Component.
</li>
<li>
<code>roleProbe</code>: Defines the procedure which is invoked regularly to assess the role of replicas.
</li>
<li>
<code>availableProbe</code>: Defines the procedure which is invoked regularly to assess the availability of the component.
</li>
<li>
<code>switchover</code>: Defines the procedure for a controlled transition of a role to a new replica.
...
<li>
<code>memberJoin</code>: Defines the procedure to add a new replica to the replication group.
</li>
<li>
<code>memberLeave</code>: Defines the method to remove a replica from the replication group.
</li>
<li>
<code>readOnly</code>: Defines the procedure to switch a replica into the read-only state.
</li>
<li>
<code>readWrite</code>: transition a replica from the read-only state back to the read-write state.
</li>
<li>
<code>dataDump</code>: Defines the procedure to export the data from a replica.
</li>
<li>
<code>dataLoad</code>: Defines the procedure to import data into a replica.
</li>
<li>
<code>reconfigure</code>: Defines the procedure that update a replica with new configuration file.
</li>
<li>
<code>accountProvision</code>: Defines the procedure to generate a new database account.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ComponentVersion">
ComponentVersion
...
<h3 id="apps.kubeblocks.io/v1.ServiceDescriptor">
ServiceDescriptor
...
in the Cluster configuration under <code>clusterComponent.serviceRefs[&ast;].serviceDescriptor</code>.
</p>
</div>
<table>
<thead>
...
- &ldquo;ch&rdquo;, &ldquo;clickhouse&rdquo;: ClickHouse service
</p>
...
<p>
If the service is exposed via a cluster, the endpoint will be provided in the format of <code>host:port</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ShardingDefinition">
ShardingDefinition
...
<p>
Specifies the strategy for provisioning shards of the sharding. Only <code>Serial</code> and <code>Parallel</code> are supported.
</p>
...
<p>
Specifies the strategy for updating shards of the sharding. Only <code>Serial</code> and <code>Parallel</code> are supported.
</p>
...
<h3 id="apps.kubeblocks.io/v1.SidecarDefinition">
SidecarDefinition
...
<h3 id="apps.kubeblocks.io/v1.Action">
Action
...
It provides a modular and extensible way to customize a Component&rsquo;s behavior through the execution of defined actions.
</p>
...
<li>
<code>readWrite</code>: Defines the procedure to transition a replica from the read-only state back to the read-write state.
</li>
<li>
<code>dataDump</code>: Defines the procedure to export the data from a replica.
</li>
...
<li>
<code>reconfigure</code>: Defines the procedure that update a replica with new configuration.
</li>
<li>
<code>accountProvision</code>: Defines the procedure to generate a new database account.
</li>
...
<p>
Used in conjunction with the <code>targetPodSelector</code> field to refine the selection of target pod(s) for Action execution.
The impact of this field depends on the <code>targetPodSelector</code> value:
</p>
<ul>
<li>
When <code>targetPodSelector</code> is set to <code>Any</code> or <code>All</code>, this field will be ignored.
</li>
<li>
When <code>targetPodSelector</code> is set to <code>Role</code>, only those replicas whose role matches the <code>matchingKey</code>
...
Currently, this is only applicable to the <code>postProvision</code> action.
</p>
...
<ul>
<li>
<code>Immediately</code>: Executed right after the Component object is created.
The readiness of the Component and its resources is not guaranteed at this stage.
</li>
<li>
<code>RuntimeReady</code>: The Action is triggered after the Component object has been created and all associated
runtime resources (e.g. Pods) are in a ready state.
</li>
<li>
<code>ComponentReady</code>: The Action is triggered after the Component itself is in a ready state.
This process does not affect the readiness state of the Component or the Cluster.
</li>
<li>
<code>ClusterReady</code>: The Action is executed after the Cluster is in a ready state.
This execution does not alter the Component or the Cluster&rsquo;s state of readiness.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ActionAssertion">
ActionAssertion
...
<h3 id="apps.kubeblocks.io/v1.ActionOutputMatcher">
ActionOutputMatcher
...
<h3 id="apps.kubeblocks.io/v1.ClusterBackup">
ClusterBackup
...
<p>
The cron expression for the schedule. The timezone is in UTC. See <a href="https://en.wikipedia.org/wiki/Cron">https://en.wikipedia.org/wiki/Cron</a>.
</p>
...
<p>
The cron expression for the incremental backup schedule. The timezone is in UTC. See <a href="https://en.wikipedia.org/wiki/Cron">https://en.wikipedia.org/wiki/Cron</a>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentConfig">
ClusterComponentConfig
...
<ul>
<li>
KB_CONFIG_FILES_CREATED: file1,file2&hellip;
</li>
<li>
KB_CONFIG_FILES_REMOVED: file1,file2&hellip;
</li>
<li>
KB_CONFIG_FILES_UPDATED: file1:checksum1,file2:checksum2&hellip;
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentConfigSource">
ClusterComponentConfigSource
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentService">
...
<p>
References the ComponentService name defined in the <code>componentDefinition.spec.services[&ast;].name</code>.
</p>
...
<p>
Determines how the Service is exposed. Valid options are <code>ClusterIP</code>, <code>NodePort</code>, and <code>LoadBalancer</code>.
</p>
<ul>
<li>
...
<li>
<code>LoadBalancer</code> builds on NodePort and creates an external load-balancer (if supported in the current cloud)
which routes to the same endpoints as the ClusterIP.
</li>
</ul>

<p>
Note: although K8s Service type allows the &lsquo;ExternalName&rsquo; type, it is not a valid option for ClusterComponentService.
</p>

<p>
For more info, see:
<a href="https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types">https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types</a>.
</p>
...
More info: <a href="https://kubernetes.io/docs/concepts/services-networking/service/#loadbalancer">https://kubernetes.io/docs/concepts/services-networking/service/#loadbalancer</a>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentSpec">
ClusterComponentSpec
...
<p>
Specifies the Component&rsquo;s name.
...
<p>
Specifies the ComponentDefinition custom resource (CR) that defines the Component&rsquo;s characteristics and behavior.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentStatus">
...
<h3 id="apps.kubeblocks.io/v1.ClusterComponentVolumeClaimTemplate">
ClusterComponentVolumeClaimTemplate
...
<ul>
<li>
<code>componentDefinition.spec.runtime.containers[&ast;].volumeMounts</code>
</li>
<li>
<code>clusterDefinition.spec.componentDefs[&ast;].podSpec.containers[&ast;].volumeMounts</code> (deprecated)
</li>
</ul>

<p>
The value of <code>name</code> must match the <code>name</code> field of a volumeMount specified in the corresponding <code>volumeMounts</code> array.
</p>
...
with the mount name specified in the <code>name</code> field.
</p>

<p>
When a Pod is created for this ClusterComponent, a new PVC will be created based on the specification
defined in the <code>spec</code> field. The PVC will be associated with the volume mount specified by the <code>name</code> field.
</p>
<br/>
<br/>
<table>
...
More info: <a href="https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1">https://kubernetes.io/docs/concepts/storage/persistent-volumes#access-modes-1</a>.
</p>
...
More info: <a href="https://kubernetes.io/docs/concepts/storage/persistent-volumes#resources">https://kubernetes.io/docs/concepts/storage/persistent-volumes#resources</a>.
</p>
...
More info: <a href="https://kubernetes.io/docs/concepts/storage/persistent-volumes#class-1">https://kubernetes.io/docs/concepts/storage/persistent-volumes#class-1</a>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterDefinitionSpec">
ClusterDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1.ClusterDefinitionStatus">
ClusterDefinitionStatus
...
<p>
Specifies the current phase of the ClusterDefinition. Valid values are <code>empty</code>, <code>Available</code>, <code>Unavailable</code>.
When <code>Available</code>, the ClusterDefinition is ready and can be referenced by related objects.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterObjectReference">
...
<h3 id="apps.kubeblocks.io/v1.ClusterPhase">
ClusterPhase
...
<p>
AbnormalClusterPhase represents some components are in <code>Failed</code> phase, indicates that the cluster is in
...
<p>
CreatingClusterPhase represents all components are in <code>Creating</code> phase.
</p>
...
<p>
FailedClusterPhase represents all components are in <code>Failed</code> phase, indicates that the cluster is unavailable.
</p>
...
<p>
RunningClusterPhase represents all components are in <code>Running</code> phase, indicates that the cluster is functioning properly.
</p>
...
<p>
StoppedClusterPhase represents all components are in <code>Stopped</code> phase, indicates that the cluster has stopped and
...
<p>
StoppingClusterPhase represents at least one component is in <code>Stopping</code> phase, indicates that the cluster is in
...
<p>
UpdatingClusterPhase represents all components are in <code>Creating</code>, <code>Running</code> or <code>Updating</code> phase, and at least one
component is in <code>Creating</code> or <code>Updating</code> phase, indicates that the cluster is undergoing an update.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterService">
ClusterService
...
using the <code>serviceRef</code> field.
</p>

<p>
When a Component needs to access another Cluster&rsquo;s ClusterService using the <code>serviceRef</code> field,
...
<p>
If the <code>componentSelector</code> is set as the name of a sharding, the service will be exposed to all components in the sharding.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterSharding">
ClusterSharding
...
would be &ldquo;my-shard-abc&rdquo;.
</p>

<p>
Note that the name defined in Component template(<code>clusterSharding.template.name</code>) will be disregarded
when generating the Component names of the shards. The <code>clusterSharding.name</code> field takes precedence.
</p>
...
<p>
Specifies the ShardingDefinition custom resource (CR) that defines the sharding&rsquo;s characteristics and behavior.
</p>
...
All the generated Components will have the same specifications and definitions as specified in the <code>template</code> field.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterSpec">
ClusterSpec
...
<h3 id="apps.kubeblocks.io/v1.ClusterStatus">
ClusterStatus
...
<code>Creating</code>, <code>Running</code>, <code>Updating</code>, <code>Stopping</code>, <code>Stopped</code>, <code>Deleting</code>, <code>Failed</code>, <code>Abnormal</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ClusterTopology">
ClusterTopology
...
<h3 id="apps.kubeblocks.io/v1.ClusterTopologyComponent">
ClusterTopologyComponent
...
<p>
Specifies the exact name, name prefix, or regular expression pattern for matching the name of the ComponentDefinition
custom resource (CR) that defines the Component&rsquo;s characteristics and behavior.
</p>
...
<ol>
<li>
Precise selection by providing the exact name of a ComponentDefinition CR.
</li>
<li>
Flexible and automatic selection of the most up-to-date ComponentDefinition CR
...
<h3 id="apps.kubeblocks.io/v1.ClusterTopologyOrders">
ClusterTopologyOrders
...
<p>
Update determines the order for updating entities&rsquo; specifications, such as image upgrades or resource scaling.
...
<h3 id="apps.kubeblocks.io/v1.ClusterTopologySharding">
ClusterTopologySharding
...
<ol>
<li>
Precise selection by providing the exact name of a ShardingDefinition CR.
</li>
<li>
Flexible and automatic selection of the most up-to-date ShardingDefinition CR
...
<h3 id="apps.kubeblocks.io/v1.ClusterVarSelector">
ClusterVarSelector
...
<h3 id="apps.kubeblocks.io/v1.ClusterVars">
ClusterVars
...
<h3 id="apps.kubeblocks.io/v1.ComponentAvailable">
ComponentAvailable
...
<h3 id="apps.kubeblocks.io/v1.ComponentAvailableCondition">
...
<h3 id="apps.kubeblocks.io/v1.ComponentAvailableExpression">
ComponentAvailableExpression
...
<h3 id="apps.kubeblocks.io/v1.ComponentAvailableProbeAssertion">
ComponentAvailableProbeAssertion
...
<h3 id="apps.kubeblocks.io/v1.ComponentAvailableWithProbe">
ComponentAvailableWithProbe
...
<h3 id="apps.kubeblocks.io/v1.ComponentDefinitionSpec">
ComponentDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1.ComponentDefinitionStatus">
ComponentDefinitionStatus
...
<p>
Represents the current status of the ComponentDefinition. Valid values include `<code>,</code>Available<code>, and</code>Unavailable<code>.
When the status is</code>Available`, the ComponentDefinition is ready and can be utilized by related objects.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentFileTemplate">
ComponentFileTemplate
...
<h3 id="apps.kubeblocks.io/v1.ComponentLifecycleActions">
ComponentLifecycleActions
...
<p>
Specifies the hook to be executed after a component&rsquo;s creation.
</p>

<p>
By setting <code>postProvision.customHandler.preCondition</code>, you can determine the specific lifecycle stage
...
with <code>ComponentReady</code> being the default.
</p>
...
which initiates an update of the replica&rsquo;s role.
</p>
...
<ul>
<li>
KB_SWITCHOVER_CANDIDATE_NAME: The name of the pod of the new role&rsquo;s candidate, which may not be specified (empty).
</li>
<li>
KB_SWITCHOVER_CANDIDATE_FQDN: The FQDN of the pod of the new role&rsquo;s candidate, which may not be specified (empty).
</li>
<li>
...
<p>
For example, to add a new OBServer to an OceanBase Cluster in &lsquo;zone1&rsquo;, the following command may be used:
</p>
<pre>
<code class="language-yaml">
command:
...
<p>
For example, to remove an OBServer from an OceanBase Cluster in &lsquo;zone1&rsquo;, the following command can be executed:
</p>
<pre>
<code class="language-yaml">
command:
...
<p>
Use Case:
This action is invoked when the database&rsquo;s volume capacity nears its upper limit and space is about to be exhausted.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentPhase">
ComponentPhase
(<code>string</code> alias)
</h3>
...
<p>
FailedComponentPhase indicates that there are some pods of the component not in a &lsquo;Running&rsquo; state.
</p>
...
<p>
RunningComponentPhase indicates that all pods of the component are up-to-date and in a &lsquo;Running&rsquo; state.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentService">
ComponentService
...
of the ComponentService using the <code>serviceVarRef</code> field.
</p>
</div>
<table>
<thead>
...
and the <code>roleSelector</code> field will be ignored.
</p>

<p>
The names of the generated Services will follow the same suffix naming pattern: <code>$(serviceName)-$(podOrdinal)</code>.
...
<h3 id="apps.kubeblocks.io/v1.ComponentSpec">
ComponentSpec
...
<h3 id="apps.kubeblocks.io/v1.ComponentStatus">
ComponentStatus
...
<ul>
<li>
Creating: The initial phase for new Components, transitioning from &lsquo;empty&rsquo;(&ldquo;&rdquo;).
</li>
<li>
...
<p>
Keys in this map are formatted as <code>ObjectKind/Name</code>, where <code>ObjectKind</code> could be a type like Pod,
and <code>Name</code> is the specific name of the object.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentSystemAccount">
ComponentSystemAccount
...
<p>
Specifies the policy for generating the account&rsquo;s password.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentVarSelector">
...
<h3 id="apps.kubeblocks.io/v1.ComponentVars">
ComponentVars
...
and the value will be presented in the following format: name1,name2,&hellip;
</p>
...
The value will be presented in the following format: FQDN1,FQDN2,&hellip;
</p>
...
The value will be presented in the following format: name1,name2,&hellip;
</p>
...
<h3 id="apps.kubeblocks.io/v1.ComponentVersionCompatibilityRule">
ComponentVersionCompatibilityRule
...
<li>
&ldquo;mysql-8.0.30&rdquo;: Matches all names starting with &ldquo;mysql-8.0.30&rdquo;
</li>
<li>
&rdquo;^mysql-8.0.\d&#123;1,2&#125;$&ldquo;: Matches all names starting with &ldquo;mysql-8.0.&rdquo; followed by one or two digits.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ComponentVersionRelease">
ComponentVersionRelease
...
<h3 id="apps.kubeblocks.io/v1.ComponentVersionSpec">
ComponentVersionSpec
...
<h3 id="apps.kubeblocks.io/v1.ComponentVersionStatus">
ComponentVersionStatus
...
<p>
Phase valid values are `<code>,</code>Available<code>, 'Unavailable</code>.
...
<h3 id="apps.kubeblocks.io/v1.ComponentVolume">
ComponentVolume
...
as defined in <code>componentDefinition.spec.lifecycleActions.readWrite</code>, restoring full functionality.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ConnectionCredentialAuth">
ConnectionCredentialAuth
...
<h3 id="apps.kubeblocks.io/v1.ContainerVars">
ContainerVars
...
<h3 id="apps.kubeblocks.io/v1.CredentialVar">
CredentialVar
...
<ul>
<li>
&rdquo;$(VAR_NAME)&rdquo; will be expanded to the value of the environment variable VAR_NAME.
</li>
<li>
&rdquo;$$(VAR_NAME)&rdquo; will result in &ldquo;$(VAR_NAME)&rdquo; in the output, without any variable expansion.
</li>
</ul>
...
<p>
Specifies the source for the variable&rsquo;s value.
</p>
...
<h3 id="apps.kubeblocks.io/v1.CredentialVarSelector">
...
<h3 id="apps.kubeblocks.io/v1.CredentialVars">
CredentialVars
...
<h3 id="apps.kubeblocks.io/v1.EnvVar">
EnvVar
...
<p>
Variable references <code>$(VAR_NAME)</code> are expanded using the previously defined variables in the current context.
</p>

<p>
If a variable cannot be resolved, the reference in the input string will be unchanged.
Double <code>$$</code> are reduced to a single <code>$</code>, which allows for escaping the <code>$(VAR_NAME)</code> syntax: i.e.
</p>
<ul>
<li>
<code>$$(VAR_NAME)</code> will produce the string literal <code>$(VAR_NAME)</code>.
</li>
</ul>
...
<p>
Source for the variable&rsquo;s value. Cannot be used if value is not empty.
</p>
...
that &ldquo;-&rdquo; is not a valid identifier in Go.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ExecAction">
ExecAction
...
These variables enable the container to adapt its behavior based on the environment it&rsquo;s running in.
</p>
...
<p>
Args represents the arguments that are passed to the <code>command</code> for execution.
</p>
...
This allows the action to utilize the specified container&rsquo;s resources without executing within it.
</p>

<p>
The name must match one of the containers defined in <code>componentDefinition.spec.runtime</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.Exporter">
Exporter
...
If empty, Prometheus uses the default value (e.g. <code>/metrics</code>).
</p>
...
If empty, Prometheus uses the default value <code>http</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.GRPCAction">
GRPCAction
...
It may be a numeric string (e.g., &ldquo;50051&rdquo;) or a named port defined in the container spec.
</p>
...
Defaults to &ldquo;127.0.0.1&rdquo; if not specified.
</p>
...
<h3 id="apps.kubeblocks.io/v1.GRPCRequest">
GRPCRequest
(<code>map[string]string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.GRPCResponse">
GRPCResponse
...
<h3 id="apps.kubeblocks.io/v1.HTTPAction">
HTTPAction
...
<p>
The port to access on the host.
It may be a numeric string (e.g., &ldquo;8080&rdquo;) or a named port defined in the container spec.
</p>
...
Defaults to &ldquo;/&rdquo; if not specified.
</p>
...
<h3 id="apps.kubeblocks.io/v1.HTTPHeader">
HTTPHeader
...
<h3 id="apps.kubeblocks.io/v1.HostNetwork">
HostNetwork
...
<h3 id="apps.kubeblocks.io/v1.HostNetworkContainerPort">
HostNetworkContainerPort
...
<h3 id="apps.kubeblocks.io/v1.HostNetworkVarSelector">
...
<h3 id="apps.kubeblocks.io/v1.HostNetworkVars">
HostNetworkVars
...
<h3 id="apps.kubeblocks.io/v1.InstanceTemplate">
InstanceTemplate
...
<p>
Specifies a map of key-value pairs to be merged into the Pod&rsquo;s existing annotations.
...
<p>
Specifies a map of key-value pairs that will be merged into the Pod&rsquo;s existing labels.
...
<h3 id="apps.kubeblocks.io/v1.InstanceUpdateStrategy">
InstanceUpdateStrategy
...
<h3 id="apps.kubeblocks.io/v1.InstanceUpdateStrategyType">
InstanceUpdateStrategyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.Issuer">
Issuer
...
It only allows two enum values: <code>KubeBlocks</code> and <code>UserProvided</code>.
</p>
<ul>
<li>
<code>KubeBlocks</code> indicates that the self-signed TLS certificates generated by the KubeBlocks Operator will be used.
</li>
<li>
<code>UserProvided</code> means that the user is responsible for providing their own CA, Cert, and Key.
...
It is required when the issuer is set to <code>UserProvided</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1.IssuerName">
IssuerName
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.LetterCase">
LetterCase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.LogConfig">
LogConfig
...
<p>
Specifies a descriptive label for the log type, such as &lsquo;slow&rsquo; for a MySQL slow log file.
It provides a clear identification of the log&rsquo;s purpose and content.
</p>
...
<h3 id="apps.kubeblocks.io/v1.MultipleClusterObjectCombinedOption">
...
<h3 id="apps.kubeblocks.io/v1.MultipleClusterObjectOption">
MultipleClusterObjectOption
...
Valid only when the strategy is set to &ldquo;combined&rdquo;.
</p>
...
<h3 id="apps.kubeblocks.io/v1.MultipleClusterObjectStrategy">
MultipleClusterObjectStrategy
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.MultipleClusterObjectValueFormat">
MultipleClusterObjectValueFormat
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.MultipleClusterObjectValueFormatFlatten">
MultipleClusterObjectValueFormatFlatten
...
<h3 id="apps.kubeblocks.io/v1.NamedVar">
NamedVar
...
<h3 id="apps.kubeblocks.io/v1.Ordinals">
Ordinals
...
<h3 id="apps.kubeblocks.io/v1.PasswordConfig">
PasswordConfig
...
use a default symbol set, which is &ldquo;!@#&amp;*&rdquo;.
</p>
...
<p>
Seed to generate the account&rsquo;s password.
Cannot be updated.
</p>
...
<h3 id="apps.kubeblocks.io/v1.PersistentVolumeClaimRetentionPolicy">
PersistentVolumeClaimRetentionPolicy
...
The default policy of <code>Delete</code> causes those PVCs to be deleted.
</p>
...
The default policy of <code>Delete</code> causes the associated PVCs for pods scaled down to be deleted.
</p>
...
<h3 id="apps.kubeblocks.io/v1.PersistentVolumeClaimRetentionPolicyType">
PersistentVolumeClaimRetentionPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.PersistentVolumeClaimSpec">
PersistentVolumeClaimSpec
...
<h3 id="apps.kubeblocks.io/v1.Phase">
Phase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.PodUpdatePolicyType">
PodUpdatePolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.PreConditionType">
PreConditionType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.Probe">
Probe
...
begins to detect the container&rsquo;s role.
</p>
...
<h3 id="apps.kubeblocks.io/v1.PrometheusScheme">
PrometheusScheme
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.ProvisionSecretRef">
ProvisionSecretRef
...
<h3 id="apps.kubeblocks.io/v1.Range">
Range
...
<h3 id="apps.kubeblocks.io/v1.ReplicaRole">
ReplicaRole
...
<p>
Name defines the role&rsquo;s unique identifier. This value is used to set the &ldquo;apps.kubeblocks.io/role&rdquo; label
...
- &ldquo;learner&rdquo;: Read-only instances that don&rsquo;t participate in elections
</p>
...
<h3 id="apps.kubeblocks.io/v1.ReplicasLimit">
ReplicasLimit
...
<h3 id="apps.kubeblocks.io/v1.ResourceVarSelector">
ResourceVarSelector
...
<h3 id="apps.kubeblocks.io/v1.ResourceVars">
ResourceVars
...
<h3 id="apps.kubeblocks.io/v1.RetryPolicy">
RetryPolicy
...
<h3 id="apps.kubeblocks.io/v1.RoledVar">
RoledVar
...
<h3 id="apps.kubeblocks.io/v1.RollingUpdate">
RollingUpdate
...
<h3 id="apps.kubeblocks.io/v1.SchedulingPolicy">
SchedulingPolicy
...
More info: <a href="https://kubernetes.io/docs/concepts/configuration/assign-pod-node/">https://kubernetes.io/docs/concepts/configuration/assign-pod-node/</a>
</p>
...
specified <code>key</code>, <code>value</code>, <code>effect</code>, and <code>operator</code>.
</p>
<ul>
<li>
The <code>key</code>, <code>value</code>, and <code>effect</code> identify the taint that the toleration matches.
</li>
<li>
The <code>operator</code> determines how the toleration matches the taint.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.Service">
Service
...
More info: <a href="https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies">https://kubernetes.io/docs/concepts/services-networking/service/#virtual-ips-and-service-proxies</a>
</p>
...
More info: <a href="https://kubernetes.io/docs/concepts/services-networking/service/">https://kubernetes.io/docs/concepts/services-networking/service/</a>
</p>
...
More info: <a href="https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types">https://kubernetes.io/docs/concepts/services-networking/service/#publishing-services-service-types</a>
</p>
...
<p>
Supports &ldquo;ClientIP&rdquo; and &ldquo;None&rdquo;. Used to maintain session affinity.
...
More info: <a href="https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/">https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/</a>
</p>
...
(<a href="https://tools.ietf.org/html/rfc1123">https://tools.ietf.org/html/rfc1123</a>) and requires <code>type</code> to be &ldquo;ExternalName&rdquo;.
</p>
...
Once set, it can not be changed. This field will be wiped when a service is updated to a non &lsquo;LoadBalancer&rsquo; type.
</p>
...
<p>
Extends the above <code>serviceSpec.selector</code> by allowing you to specify defined role as selector for the service.
...
<p>
In this example, setting <code>roleSelector</code> to &ldquo;leader&rdquo; will add a label selector
...
&ldquo;kubeblocks.io/role&rdquo; set to &ldquo;leader&rdquo;.
</p>

<p>
Note that if <code>podService</code> sets to true, RoleSelector will be ignored.
The <code>podService</code> flag takes precedence over <code>roleSelector</code> and generates a service for each Pod.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceDescriptorSpec">
ServiceDescriptorSpec
...
<h3 id="apps.kubeblocks.io/v1.ServiceDescriptorStatus">
ServiceDescriptorStatus
...
<p>
Indicates the current lifecycle phase of the ServiceDescriptor. This can be either &lsquo;Available&rsquo; or &lsquo;Unavailable&rsquo;.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceRef">
ServiceRef
...
<ul>
<li>
<code>componentDefinition.spec.serviceRefDeclarations[&ast;].name</code>
</li>
<li>
<code>clusterDefinition.spec.componentDefs[&ast;].serviceRefDeclarations[&ast;].name</code> (deprecated)
</li>
</ul>
...
<p>
By default, the referenced KubeBlocks Cluster&rsquo;s <code>clusterDefinition.spec.connectionCredential</code>
will be utilized to bind to the current Component. This credential should include:
<code>endpoint</code>, <code>port</code>, <code>username</code>, and <code>password</code>.
</p>
...
<li>
If both <code>cluster</code> and <code>serviceDescriptor</code> are present, <code>cluster</code> will take precedence.
</li>
</ul>

<p>
Deprecated since v0.9 since <code>clusterDefinition.spec.connectionCredential</code> is deprecated,
...
<p>
If both <code>cluster</code> and <code>serviceDescriptor</code> are specified, the <code>cluster</code> takes precedence.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefClusterSelector">
ServiceRefClusterSelector
...
<p>
Identifies a ClusterService from the list of Services defined in <code>cluster.spec.services</code> of the referenced Cluster.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefCredentialSelector">
ServiceRefCredentialSelector
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefDeclaration">
ServiceRefDeclaration
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefDeclarationSpec">
ServiceRefDeclarationSpec
...
For instance, <code>^8.0.8$</code>, <code>8.0.\d&#123;1,2&#125;$</code>, <code>^[v\-]&ast;?(\d&#123;1,2&#125;\.)&#123;0,3&#125;\d&#123;1,2&#125;$</code> are all valid patterns.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefPodFQDNsSelector">
ServiceRefPodFQDNsSelector
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefServiceSelector">
ServiceRefServiceSelector
...
<p>
It is required when referencing a Component&rsquo;s Service.
</p>
...
<p>
Leave it empty to reference the default Service. Set it to &ldquo;headless&rdquo; to reference the default headless Service.
</p>

<p>
If the referenced Service is of pod-service type (a Service per Pod), there will be multiple Service objects matched,
and the resolved value will be presented in the following format: service1.name,service2.name&hellip;
</p>
...
<p>
If the referenced Service is of pod-service type (a Service per Pod), there will be multiple Service objects matched,
and the resolved value will be presented in the following format: service1.name:port1,service2.name:port2&hellip;
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefVarSelector">
...
<h3 id="apps.kubeblocks.io/v1.ServiceRefVars">
ServiceRefVars
...
<h3 id="apps.kubeblocks.io/v1.ServiceVarSelector">
ServiceVarSelector
...
It can be referenced from the default headless service by setting the name to &ldquo;headless&rdquo;.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ServiceVars">
ServiceVars
...
and the value will be presented in the following format: service1.name:port1,service2.name:port2&hellip;
</p>
...
<h3 id="apps.kubeblocks.io/v1.ShardingDefinitionSpec">
ShardingDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1.ShardingDefinitionStatus">
ShardingDefinitionStatus
...
<p>
Represents the current status of the ShardingDefinition. Valid values include `<code>,</code>Available<code>, and</code>Unavailable<code>.
When the status is</code>Available`, the ShardingDefinition is ready and can be utilized by related objects.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ShardingLifecycleActions">
ShardingLifecycleActions
...
<p>
Specifies the hook to be executed after a sharding&rsquo;s creation.
</p>

<p>
By setting <code>postProvision.preCondition</code>, you can determine the specific lifecycle stage at which
the action should trigger, available conditions for sharding include: <code>Immediately</code>, <code>ComponentReady</code>,
and <code>ClusterReady</code>. For sharding, the <code>ComponentReady</code> condition means all components of the sharding are ready.
</p>
...
<h3 id="apps.kubeblocks.io/v1.ShardingSystemAccount">
ShardingSystemAccount
...
<h3 id="apps.kubeblocks.io/v1.ShardingTLS">
ShardingTLS
...
<h3 id="apps.kubeblocks.io/v1.ShardingTemplate">
ShardingTemplate
...
<li>
the regular expression pattern (&lsquo;^&rsquo; will be added to the beginning of the pattern automatically)
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1.ShardsLimit">
ShardsLimit
...
<h3 id="apps.kubeblocks.io/v1.Sidecar">
Sidecar
...
<h3 id="apps.kubeblocks.io/v1.SidecarDefinitionSpec">
SidecarDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1.SidecarDefinitionStatus">
SidecarDefinitionStatus
...
<p>
Represents the current status of the SidecarDefinition. Valid values include `<code>,</code>Available<code>, and</code>Unavailable<code>.
When the status is</code>Available`, the SidecarDefinition is ready and can be utilized by related objects.
</p>
...
<h3 id="apps.kubeblocks.io/v1.SystemAccount">
SystemAccount
...
<h3 id="apps.kubeblocks.io/v1.SystemAccountStatement">
SystemAccountStatement
...
<h3 id="apps.kubeblocks.io/v1.TLS">
TLS
...
<h3 id="apps.kubeblocks.io/v1.TLSConfig">
TLSConfig
...
<h3 id="apps.kubeblocks.io/v1.TLSSecretRef">
TLSSecretRef
...
<h3 id="apps.kubeblocks.io/v1.TLSVarSelector">
TLSVarSelector
...
<h3 id="apps.kubeblocks.io/v1.TLSVars">
TLSVars
...
<h3 id="apps.kubeblocks.io/v1.TargetPodSelector">
TargetPodSelector
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.TerminationPolicyType">
TerminationPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.UpdateStrategy">
UpdateStrategy
(<code>string</code> alias)
</h3>
...
across the cluster.
The available strategies are <code>Serial</code>, <code>BestEffortParallel</code>, and <code>Parallel</code>.
</p>
</div>
<table>
<thead>
...
<p>
The <code>BestEffortParallel</code> strategy strikes a balance between update speed and component availability.
</p>
...
<h3 id="apps.kubeblocks.io/v1.VarOption">
VarOption
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1.VarSource">
VarSource
...
<h3 id="apps.kubeblocks.io/v1alpha1.Cluster">
Cluster
...
<li>
<code>Halt</code>: Deletes Cluster resources like Pods and Services but retains Persistent Volume Claims (PVCs),
...
<li>
<code>Delete</code>: Extends the <code>Halt</code> policy by also removing PVCs, leading to a thorough cleanup while
...
<p>
This field supports dynamic resharding by facilitating the addition or removal of shards
through the <code>shards</code> field in ShardingSpec.
</p>

<p>
Note: <code>shardingSpecs</code> and <code>componentSpecs</code> cannot both be empty; at least one must be defined to configure a Cluster.
</p>
...
<p>
Defines a set of node affinity scheduling rules for the Cluster&rsquo;s Pods.
This field helps control the placement of Pods on nodes within the Cluster.
</p>

<p>
Deprecated since v0.10. Use the <code>schedulingPolicy</code> field instead.
</p>
...
<p>
An array that specifies tolerations attached to the Cluster&rsquo;s Pods,
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinition">
ClusterDefinition
...
<li>
<code>$(STRONG_RANDOM_PASSWD)</code> random 16 characters, with mixed cases, digits and symbols.
</li>
<li>
<code>$(UUID)</code> generate a random UUID v4 string.
</li>
<li>
<code>$(UUID_B64)</code> generate a random UUID v4 BASE64 encoded string.
</li>
<li>
<code>$(UUID_STR_B64)</code> generate a random UUID v4 string then BASE64 encoded.
</li>
<li>
<code>$(UUID_HEX)</code> generate a random UUID v4 HEX representation.
</li>
<li>
<code>$(HEADLESS_SVC_FQDN)</code> headless service FQDN placeholder, value pattern is <code>$(CLUSTER_NAME)-$(1ST_COMP_NAME)-headless.$(NAMESPACE).svc</code>,
where 1ST_COMP_NAME is the 1st component that provide <code>ClusterDefinition.spec.componentDefs[].service</code> attribute;
</li>
<li>
<code>$(SVC_FQDN)</code> service FQDN placeholder, value pattern is <code>$(CLUSTER_NAME)-$(1ST_COMP_NAME).$(NAMESPACE).svc</code>,
where 1ST_COMP_NAME is the 1st component that provide <code>ClusterDefinition.spec.componentDefs[].service</code> attribute;
</li>
<li>
<code>$(SVC_PORT_&#123;PORT-NAME&#125;)</code> is ServicePort&rsquo;s port value with specified port name, i.e, a servicePort JSON struct:
...
<h3 id="apps.kubeblocks.io/v1alpha1.Component">
Component
...
The log types are defined in the <code>componentDefinition.spec.logConfigs</code> field with the LogConfig entries.
</p>

<p>
The elements in the <code>enabledLogs</code> array correspond to the names of the LogConfig entries.
...
potentially revising the naming to &ldquo;kb-&#123;cluster.name&#125;-&#123;component.name&#125;&rdquo;.
</p>
...
It allows users to control how the Component&rsquo;s Pods are scheduled onto nodes in the Cluster.
</p>

<p>
Deprecated since v0.10, replaced by the <code>schedulingPolicy</code> field.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentDefinition">
ComponentDefinition
...
<p>
This field specifies a list of templates that will be rendered into Component containers&rsquo; configuration files.
...
This action returns a list of roles for each replica, and the returned roles must be predefined in the <code>roles</code> field.
</p>
...
<li>
<code>switchover</code>: Defines the procedure for a controlled transition of leadership from the current leader to a new replica.
This approach aims to minimize downtime and maintain availability in systems with a leader-follower topology,
such as before planned maintenance or upgrades on the current leader node.
</li>
<li>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVersion">
ComponentVersion
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigConstraint">
ConfigConstraint
...
<ol>
<li>
<strong>Parameter Value Validation</strong>: Validates and ensures compliance of parameter values with defined constraints.
</li>
<li>
<strong>Dynamic Reload on Modification</strong>: Monitors parameter changes and triggers dynamic reloads to apply updates.
</li>
<li>
<strong>Parameter Rendering in Templates</strong>: Injects parameters into templates to generate up-to-date configuration files.
</li>
</ol>
</div>
<table>
...
<ol>
<li>
The modified parameters are listed in the <code>dynamicParameters</code> field.
...
<li>
<code>reloadOptions</code> is set.
</li>
</ol>

<p>
If <code>reloadOptions</code> is not set or the modified parameters are not listed in <code>dynamicParameters</code>,
...
<p>
Configures whether the dynamic reload specified in <code>reloadOptions</code> applies only to dynamic parameters or
to all parameters (including static parameters).
</p>
<ul>
<li>
false (default): Only modifications to the dynamic parameters listed in <code>dynamicParameters</code>
will trigger a dynamic reload.
</li>
<li>
true: Modifications to both dynamic parameters listed in <code>dynamicParameters</code> and static parameters
...
This ensures that the tools are available to the &lsquo;config-manager&rsquo; sidecar.
</p>
...
<p>
Specifies the top-level key in the &lsquo;configurationSchema.cue&rsquo; that organizes the validation rules for parameters.
This key must exist within the CUE script defined in &lsquo;configurationSchema.cue&rsquo;.
</p>
...
The <code>selector</code> allows you to specify label selectors to target the desired pods for the reload process.
</p>

<p>
If the <code>selector</code> is not specified or is nil, all pods managed by the workload will be considered for the dynamic
reload.
</p>
...
Supported formats include <code>ini</code>, <code>xml</code>, <code>yaml</code>, <code>json</code>, <code>hcl</code>, <code>dotenv</code>, <code>properties</code>, and <code>toml</code>.
</p>

<p>
Each format may have its own set of parameters that can be configured.
For instance, when using the <code>ini</code> format, you can specify the section name.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.Configuration">
Configuration
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceDescriptor">
ServiceDescriptor
...
<h3 id="apps.kubeblocks.io/v1alpha1.AccessMode">
AccessMode
(<code>string</code> alias)
</h3>
...
The modes can be <code>None</code>, <code>Readonly</code>, or <code>ReadWrite</code>.
</p>
</div>
<table>
<thead>
...
<h3 id="apps.kubeblocks.io/v1alpha1.AccountName">
AccountName
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.Action">
Action
...
<p>
When specified, a dedicated container will be created using this image to execute the Action.
This field is mutually exclusive with the <code>container</code> field; only one of them should be provided.
</p>
...
<p>
This name must correspond to one of the containers defined in <code>componentDefinition.spec.runtime</code>.
If this field is not specified, the default behavior is to use the first container listed in
<code>componentDefinition.spec.runtime</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.Affinity">
Affinity
...
<p>
For example, if NodeLabels is set to &#123;&ldquo;nodeType&rdquo;: &ldquo;ssd&rdquo;, &ldquo;environment&rdquo;: &ldquo;production&rdquo;&#125;,
pods will only be scheduled on nodes that have both the &ldquo;nodeType&rdquo; label with value &ldquo;ssd&rdquo;
and the &ldquo;environment&rdquo; label with value &ldquo;production&rdquo;.
</p>
...
It can have the following values: <code>SharedNode</code> and <code>DedicatedNode</code>.
</p>
<ul>
<li>
...
<p>
The default value is <code>SharedNode</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.AvailabilityPolicyType">
AvailabilityPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.BackupStatusUpdateStage">
...
<h3 id="apps.kubeblocks.io/v1alpha1.BaseBackupType">
BaseBackupType
(<code>string</code> alias)
</h3>
<div>
...
<h3 id="apps.kubeblocks.io/v1alpha1.BuiltinActionHandlerType">
...
<li>
<code>vanilla-postgresql</code>
</li>
<li>
<code>apecloud-postgresql</code>
</li>
<li>
<code>polardbx</code>
</li>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClassDefRef">
ClassDefRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterBackup">
ClusterBackup
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentConfig">
ClusterComponentConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentConfigSource">
ClusterComponentConfigSource
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentDefinition">
ClusterComponentDefinition
...
<p>
This name could be used as default name of <code>cluster.spec.componentSpecs.name</code>, and needs to conform with same
validation rules as <code>cluster.spec.componentSpecs.name</code>, currently complying with IANA Service Naming rule.
This name will apply to cluster objects as the value of label &ldquo;apps.kubeblocks.io/component-name&rdquo;.
</p>
...
<ul>
<li>
<code>Stateless</code> describes stateless applications.
</li>
<li>
<code>Stateful</code> describes common stateful applications.
</li>
<li>
<code>Consensus</code> describes applications based on consensus protocols, such as raft and paxos.
</li>
<li>
<code>Replication</code> describes applications based on the primary-secondary data replication protocol.
</li>
</ul>
...
<p>
Defines spec for <code>Stateless</code> workloads.
</p>
...
<p>
Defines spec for <code>Stateful</code> workloads.
</p>
...
<p>
Defines spec for <code>Consensus</code> workloads. It&rsquo;s required if the workload type is <code>Consensus</code>.
</p>
...
<p>
Defines spec for <code>Replication</code> workloads.
</p>
...
<li>
<code>name: binlog, type: log</code> means that the volume named <code>binlog</code> is used to store <code>log</code>.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentPhase">
...
in a &lsquo;Running&rsquo; state.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentService">
ClusterComponentService
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentSpec">
ClusterComponentSpec
...
<p>
References a ClusterComponentDefinition defined in the <code>clusterDefinition.spec.componentDef</code> field.
...
the <code>componentDef</code> will take precedence over <code>componentDefRef</code>.
</p>
...
<p>
Specifies a group of affinity scheduling rules for the Component.
It allows users to control how the Component&rsquo;s Pods are scheduled onto nodes in the K8s cluster.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentStatus">
ClusterComponentStatus
...
<p>
Records detailed information about the Component in its current phase.
The keys are either podName, deployName, or statefulSetName, formatted as &lsquo;ObjectKind/Name&rsquo;.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterComponentVolumeClaimTemplate">
ClusterComponentVolumeClaimTemplate
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinitionProbe">
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinitionProbeCMDs">
ClusterDefinitionProbeCMDs
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinitionProbes">
ClusterDefinitionProbes
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinitionSpec">
ClusterDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterDefinitionStatus">
ClusterDefinitionStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterNetwork">
ClusterNetwork
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterObjectReference">
ClusterObjectReference
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterPhase">
ClusterPhase
(<code>string</code> alias)
</h3>
...
<p>
AbnormalClusterPhase represents some components are in <code>Failed</code> or <code>Abnormal</code> phase, indicates that the cluster
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterResources">
ClusterResources
...
For more information, refer to: <a href="https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/">https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/</a>
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterService">
ClusterService
...
<code>cluster.spec.shardingSpecs[&ast;].name</code>, to be used as a selector for the service.
Note that this and the <code>componentSelector</code> are mutually exclusive and cannot be set simultaneously.
</p>
...
Note that this and the <code>shardingSelector</code> are mutually exclusive and cannot be set simultaneously.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterSpec">
ClusterSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterStatus">
ClusterStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterStorage">
ClusterStorage
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterSwitchPolicy">
ClusterSwitchPolicy
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterTopology">
ClusterTopology
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterTopologyComponent">
ClusterTopologyComponent
...
<h3 id="apps.kubeblocks.io/v1alpha1.ClusterTopologyOrders">
ClusterTopologyOrders
...
<p>
Update determines the order for updating components&rsquo; specifications, such as image upgrades or resource scaling.
...
<h3 id="apps.kubeblocks.io/v1alpha1.CmdExecutorConfig">
CmdExecutorConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.CommandExecutorEnvItem">
CommandExecutorEnvItem
...
<h3 id="apps.kubeblocks.io/v1alpha1.CommandExecutorItem">
CommandExecutorItem
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentConfigSpec">
ComponentConfigSpec
...
<p>
The template is rendered inside the pod (by the &ldquo;config-manager&rdquo; sidecar container) and merged with the main
template&rsquo;s render result to generate the final configuration file.
</p>
...
<code>cluster.spec.componentSpecs[&ast;].instances[&ast;]</code>.
</p>
...
<p>
Deprecated: <code>asEnvFrom</code> has been deprecated since 0.9.0 and will be removed in 0.10.0.
Use <code>injectEnvTo</code> instead.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentDefRef">
ComponentDefRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentDefinitionSpec">
ComponentDefinitionSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentDefinitionStatus">
ComponentDefinitionStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentLifecycleActions">
ComponentLifecycleActions
...
<p>
KB_CLUSTER_POD_IP_LIST: Comma-separated list of the cluster&rsquo;s pod IP addresses (e.g., &ldquo;podIp1,podIp2&rdquo;).
</p>
</li>
<li>

<p>
KB_CLUSTER_POD_NAME_LIST: Comma-separated list of the cluster&rsquo;s pod names (e.g., &ldquo;pod1,pod2&rdquo;).
</p>
</li>
<li>
...
KB_CLUSTER_POD_NAME_LIST (e.g., &ldquo;hostName1,hostName2&rdquo;).
</p>
</li>
<li>
...
KB_CLUSTER_POD_NAME_LIST (e.g., &ldquo;hostIp1,hostIp2&rdquo;).
</p>
</li>
<li>
...
matching the order of pods in KB_CLUSTER_COMPONENT_POD_NAME_LIST (e.g., &ldquo;podIp1,podIp2&rdquo;).
</p>
</li>
<li>
...
matching the order of pods in KB_CLUSTER_COMPONENT_POD_NAME_LIST (e.g., &ldquo;hostName1,hostName2&rdquo;).
</p>
</li>
<li>
...
matching the order of pods in KB_CLUSTER_COMPONENT_POD_NAME_LIST (e.g., &ldquo;hostIp1,hostIp2&rdquo;).
</p>
</li>
<li>

<p>
KB_CLUSTER_COMPONENT_LIST: Comma-separated list of all cluster components (e.g., &ldquo;comp1,comp2&rdquo;).
</p>
</li>
<li>
...
<li>
KB_SWITCHOVER_CANDIDATE_FQDN: The FQDN of the new leader candidate&rsquo;s pod, which may not be specified (empty).
</li>
<li>
KB_LEADER_POD_IP: The IP address of the current leader&rsquo;s pod prior to the switchover.
</li>
<li>
KB_LEADER_POD_NAME: The name of the current leader&rsquo;s pod prior to the switchover.
</li>
<li>
KB_LEADER_POD_FQDN: The FQDN of the current leader&rsquo;s pod prior to the switchover.
</li>
</ul>
...
<ul>
<li>
KB_REPLICATION_PRIMARY<em>POD</em>
</li>
<li>
KB_CONSENSUS_LEADER<em>POD</em>
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentMessageMap">
ComponentMessageMap
(<code>map[string]string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentRefEnv">
ComponentRefEnv
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentService">
ComponentService
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentSpec">
ComponentSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentStatus">
ComponentStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentSwitchover">
ComponentSwitchover
...
If this field is set, the scripts defined under the &lsquo;scripts&rsquo; field can be invoked or referenced within an Action.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentSystemAccount">
ComponentSystemAccount
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentTemplateSpec">
ComponentTemplateSpec
...
An empty namespace is equivalent to the &ldquo;default&rdquo; namespace.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentValueFrom">
ComponentValueFrom
...
<p>
Specifies the source to select. It can be one of three types: <code>FieldRef</code>, <code>ServiceRef</code>, <code>HeadlessServiceRef</code>.
</p>
...
<p>
The jsonpath of the source to select when the Type is <code>FieldRef</code>.
Two objects are registered in the jsonpath: <code>componentDef</code> and <code>components</code>:
</p>
<ul>
<li>
<code>componentDef</code> is the component definition object specified in <code>componentRef.componentDefName</code>.
</li>
<li>
<code>components</code> are the component list objects referring to the component definition object.
</li>
</ul>
...
Three builtin variables can be used as placeholders: <code>$POD_ORDINAL</code>, <code>$POD_FQDN</code>, <code>$POD_NAME</code>
</p>
<ul>
<li>
<code>$POD_ORDINAL</code> represents the ordinal of the pod.
</li>
<li>
<code>$POD_FQDN</code> represents the fully qualified domain name of the pod.
</li>
<li>
<code>$POD_NAME</code> represents the name of the pod.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentValueFromType">
ComponentValueFromType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVarSelector">
ComponentVarSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVars">
ComponentVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVersionCompatibilityRule">
ComponentVersionCompatibilityRule
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVersionRelease">
ComponentVersionRelease
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVersionSpec">
ComponentVersionSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVersionStatus">
ComponentVersionStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ComponentVolume">
ComponentVolume
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigConstraintSpec">
ConfigConstraintSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigConstraintStatus">
ConfigConstraintStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigMapRef">
ConfigMapRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigParams">
ConfigParams
...
Refer to <a href="https://github.com/kubernetes-sigs/kubebuilder/issues/528">https://github.com/kubernetes-sigs/kubebuilder/issues/528</a> and <a href="https://github.com/kubernetes/code-generator/issues/50">https://github.com/kubernetes/code-generator/issues/50</a> for more details.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigTemplateExtension">
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigurationItemDetail">
ConfigurationItemDetail
...
<p>
Deprecated: No longer used. Please use &lsquo;Payload&rsquo; instead. Previously represented the version of the configuration template.
</p>
...
<p>
Note: Currently, the <code>payload</code> field is opaque and its content is not interpreted by the system.
...
<p>
When provided, the <code>importTemplateRef</code> overrides the default configuration template
...
<p>
When provided, the parameter values in <code>configFileParams</code> override the default configuration parameters.
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigurationItemDetailStatus">
...
&ldquo;Upgrading&rdquo;, &ldquo;Deleting&rdquo;, &ldquo;FailedAndRetry&rdquo;, &ldquo;Finished&rdquo;.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigurationPhase">
ConfigurationPhase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigurationSpec">
ConfigurationSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConfigurationStatus">
ConfigurationStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConnectionCredentialAuth">
ConnectionCredentialAuth
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConsensusMember">
ConsensusMember
...
The default is 1 for <code>Leader</code>, 0 for <code>Learner</code>, others for <code>Followers</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ConsensusSetSpec">
ConsensusSetSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ContainerVars">
ContainerVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.CredentialVar">
CredentialVar
...
<h3 id="apps.kubeblocks.io/v1alpha1.CredentialVarSelector">
CredentialVarSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.CredentialVars">
CredentialVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.CustomLabelSpec">
CustomLabelSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.CustomParametersValidation">
CustomParametersValidation
...
<p>
Generated from the &lsquo;cue&rsquo; field and transformed into a JSON format.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.EnvVar">
EnvVar
...
<h3 id="apps.kubeblocks.io/v1alpha1.ExecAction">
ExecAction
...
<h3 id="apps.kubeblocks.io/v1alpha1.Exporter">
Exporter
...
<h3 id="apps.kubeblocks.io/v1alpha1.ExporterConfig">
ExporterConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.FailurePolicyType">
FailurePolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.GVKResource">
GVKResource
...
<p>
Represents the GVK of a resource, such as &ldquo;v1/Pod&rdquo;, &ldquo;apps/v1/StatefulSet&rdquo;, etc.
...
<h3 id="apps.kubeblocks.io/v1alpha1.HScaleDataClonePolicyType">
...
The policy can be set to <code>None</code>, <code>CloneVolume</code>, or <code>Snapshot</code>.
</p>
</div>
<table>
<thead>
...
<h3 id="apps.kubeblocks.io/v1alpha1.HTTPAction">
HTTPAction
...
<p>
Indicates the server&rsquo;s domain name or IP address. Defaults to the Pod&rsquo;s IP.
Prefer setting the &ldquo;Host&rdquo; header in httpHeaders when needed.
</p>
...
<p>
Represents the type of HTTP request to be made, such as &ldquo;GET,&rdquo; &ldquo;POST,&rdquo; &ldquo;PUT,&rdquo; etc.
If not specified, &ldquo;GET&rdquo; is the default method.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.HorizontalScalePolicy">
HorizontalScalePolicy
...
The policy can be one of the following: &#123;None, CloneVolume&#125;. The default policy is <code>None</code>.
</p>
<ul>
<li>
<code>None</code>: This is the default policy. It creates an empty volume without data cloning.
</li>
<li>
<code>CloneVolume</code>: This policy clones data to newly scaled pods. It first tries to use a volume snapshot.
If volume snapshot is not enabled, it will attempt to use a backup tool. If neither method works, it will report an error.
</li>
<li>
<code>Snapshot</code>: This policy is deprecated and is an alias for CloneVolume.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1alpha1.HostNetwork">
HostNetwork
...
<h3 id="apps.kubeblocks.io/v1alpha1.HostNetworkContainerPort">
HostNetworkContainerPort
...
<h3 id="apps.kubeblocks.io/v1alpha1.HostNetworkVarSelector">
HostNetworkVarSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.HostNetworkVars">
HostNetworkVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.InstanceTemplate">
InstanceTemplate
...
<p>
Specifies an override for the first container&rsquo;s image in the Pod.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.InstanceUpdateStrategy">
InstanceUpdateStrategy
...
<h3 id="apps.kubeblocks.io/v1alpha1.Issuer">
Issuer
...
<h3 id="apps.kubeblocks.io/v1alpha1.IssuerName">
IssuerName
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.LegacyRenderedTemplateSpec">
LegacyRenderedTemplateSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.LetterCase">
LetterCase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.LifecycleActionHandler">
LifecycleActionHandler
...
<code>postgresql</code>, <code>vanilla-postgresql</code>, <code>apecloud-postgresql</code>, <code>wesql</code>, <code>oceanbase</code>, <code>polardbx</code>.
</p>

<p>
If the <code>builtinHandler</code> field is specified, it instructs Lorry to utilize its internal built-in action handler
to execute the specified lifecycle actions.
</p>

<p>
The <code>builtinHandler</code> field is of type <code>BuiltinActionHandlerType</code>,
...
for all other actions throughout the entire <code>ComponentLifecycleActions</code> collection.
</p>
...
you can use the <code>customHandler</code> field to define your own action implementation.
</p>
...
<ul>
<li>
In the future, the <code>builtinHandler</code> field will be deprecated in favor of using the <code>customHandler</code> field
...
as developers can create their own &ldquo;builtin&rdquo; implementations tailored to their specific requirements.
</li>
</ul>
...
thereby accommodating unique logic for different database systems within the Action&rsquo;s framework.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.LogConfig">
LogConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.MergedPolicy">
MergedPolicy
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.MonitorConfig">
MonitorConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.MultipleClusterObjectCombinedOption">
MultipleClusterObjectCombinedOption
...
<h3 id="apps.kubeblocks.io/v1alpha1.MultipleClusterObjectOption">
MultipleClusterObjectOption
...
<h3 id="apps.kubeblocks.io/v1alpha1.MultipleClusterObjectStrategy">
MultipleClusterObjectStrategy
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.MultipleClusterObjectValueFormat">
MultipleClusterObjectValueFormat
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.MultipleClusterObjectValueFormatFlatten">
MultipleClusterObjectValueFormatFlatten
...
<h3 id="apps.kubeblocks.io/v1alpha1.NamedVar">
NamedVar
...
<h3 id="apps.kubeblocks.io/v1alpha1.PasswordConfig">
PasswordConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.Payload">
Payload
...
<h3 id="apps.kubeblocks.io/v1alpha1.PersistentVolumeClaimSpec">
PersistentVolumeClaimSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.Phase">
Phase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.PodAntiAffinity">
PodAntiAffinity
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.PodAvailabilityPolicy">
...
<h3 id="apps.kubeblocks.io/v1alpha1.PostStartAction">
PostStartAction
...
<h3 id="apps.kubeblocks.io/v1alpha1.PreConditionType">
PreConditionType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.Probe">
Probe
...
<h3 id="apps.kubeblocks.io/v1alpha1.PrometheusScheme">
PrometheusScheme
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProtectedVolume">
ProtectedVolume
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProvisionPolicy">
ProvisionPolicy
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProvisionPolicyType">
ProvisionPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProvisionScope">
ProvisionScope
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProvisionSecretRef">
ProvisionSecretRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.ProvisionStatements">
ProvisionStatements
...
<h3 id="apps.kubeblocks.io/v1alpha1.RSMSpec">
RSMSpec
...
<ul>
<li>
<code>Serial</code>: Updates Members sequentially to ensure minimum component downtime.
</li>
<li>
<code>BestEffortParallel</code>: Updates Members in parallel to ensure minimum component write downtime.
</li>
<li>
<code>Parallel</code>: Forces parallel updates.
</li>
</ul>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ReconcileDetail">
ReconcileDetail
...
<h3 id="apps.kubeblocks.io/v1alpha1.ReloadOptions">
ReloadOptions
...
<h3 id="apps.kubeblocks.io/v1alpha1.ReplicaRole">
ReplicaRole
...
<p>
Defines the role&rsquo;s identifier. It is used to set the &ldquo;apps.kubeblocks.io/role&rdquo; label value
...
<h3 id="apps.kubeblocks.io/v1alpha1.ReplicasLimit">
ReplicasLimit
...
<h3 id="apps.kubeblocks.io/v1alpha1.ReplicationSetSpec">
ReplicationSetSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.RerenderResourceType">
RerenderResourceType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ResourceMeta">
ResourceMeta
...
<h3 id="apps.kubeblocks.io/v1alpha1.RetryPolicy">
RetryPolicy
...
<h3 id="apps.kubeblocks.io/v1alpha1.RoleArbitrator">
RoleArbitrator
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.RoleProbe">
RoleProbe
...
<h3 id="apps.kubeblocks.io/v1alpha1.SchedulingPolicy">
SchedulingPolicy
...
<h3 id="apps.kubeblocks.io/v1alpha1.ScriptSpecSelector">
ScriptSpecSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.SecretRef">
SecretRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.Service">
Service
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceDescriptorSpec">
ServiceDescriptorSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceDescriptorStatus">
ServiceDescriptorStatus
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServicePort">
ServicePort
...
<p>
The IP protocol for this port. Supports &ldquo;TCP&rdquo;, &ldquo;UDP&rdquo;, and &ldquo;SCTP&rdquo;.
Default is TCP.
</p>
...
<ul>
<li>
If this is a string, it will be looked up as a named port in the target Pod&rsquo;s container ports.
</li>
<li>
If this is not specified, the value of the <code>port</code> field is used (an identity map).
</li>
</ul>

<p>
This field is ignored for services with clusterIP=None, and should be
omitted or set equal to the <code>port</code> field.
</p>

<p>
More info: <a href="https://kubernetes.io/docs/concepts/services-networking/service/#defining-a-service">https://kubernetes.io/docs/concepts/services-networking/service/#defining-a-service</a>
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRef">
ServiceRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefClusterSelector">
ServiceRefClusterSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefCredentialSelector">
ServiceRefCredentialSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefDeclaration">
ServiceRefDeclaration
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefDeclarationSpec">
ServiceRefDeclarationSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefServiceSelector">
ServiceRefServiceSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefVarSelector">
ServiceRefVarSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceRefVars">
ServiceRefVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceSpec">
ServiceSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceVarSelector">
ServiceVarSelector
...
<h3 id="apps.kubeblocks.io/v1alpha1.ServiceVars">
ServiceVars
...
<h3 id="apps.kubeblocks.io/v1alpha1.ShardingSpec">
ShardingSpec
...
<p>
Note that the name defined in Component template(<code>shardingSpec.template.name</code>) will be disregarded
when generating the Component names of the shards. The <code>shardingSpec.name</code> field takes precedence.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.StatefulSetSpec">
StatefulSetSpec
...
For workloadType=<code>Consensus</code>, the update strategy can be one of the following:
</p>
<ul>
<li>
<code>Serial</code>: Updates Members sequentially to minimize component downtime.
</li>
<li>
<code>BestEffortParallel</code>: Updates Members in parallel to minimize component write downtime. Majority remains online
at all times.
</li>
<li>
<code>Parallel</code>: Forces parallel updates.
...
<code>UpdateStrategy</code> will be ignored if this is provided.
</p>
...
<h3 id="apps.kubeblocks.io/v1alpha1.StatefulSetWorkload">
StatefulSetWorkload
...
<h3 id="apps.kubeblocks.io/v1alpha1.StatelessSetSpec">
StatelessSetSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.SwitchPolicyType">
SwitchPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.SwitchoverAction">
SwitchoverAction
...
<h3 id="apps.kubeblocks.io/v1alpha1.SwitchoverSpec">
SwitchoverSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.SystemAccount">
SystemAccount
...
<h3 id="apps.kubeblocks.io/v1alpha1.SystemAccountConfig">
SystemAccountConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.SystemAccountSpec">
SystemAccountSpec
...
<p>
Deprecated since v0.8, be replaced by <code>componentDefinition.spec.systemAccounts</code> and
<code>componentDefinition.spec.lifecycleActions.accountProvision</code>.
</p>
</div>
<table>
<thead>
...
<h3 id="apps.kubeblocks.io/v1alpha1.TLSConfig">
TLSConfig
...
<h3 id="apps.kubeblocks.io/v1alpha1.TLSSecretRef">
TLSSecretRef
...
<h3 id="apps.kubeblocks.io/v1alpha1.TargetPodSelector">
TargetPodSelector
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.TenancyType">
TenancyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.TerminationPolicyType">
TerminationPolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.UpdateStrategy">
UpdateStrategy
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.UpgradePolicy">
UpgradePolicy
(<code>string</code> alias)
</h3>
<div>
...
<h3 id="apps.kubeblocks.io/v1alpha1.UserResourceRefs">
UserResourceRefs
...
<h3 id="apps.kubeblocks.io/v1alpha1.VarOption">
VarOption
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.VarSource">
VarSource
...
<h3 id="apps.kubeblocks.io/v1alpha1.VolumeProtectionSpec">
VolumeProtectionSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.VolumeType">
VolumeType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1alpha1.VolumeTypeSpec">
VolumeTypeSpec
...
<h3 id="apps.kubeblocks.io/v1alpha1.WorkloadType">
WorkloadType
(<code>string</code> alias)
</h3>
...
It can be one of the following: <code>Stateless</code>, <code>Stateful</code>, <code>Consensus</code>, or <code>Replication</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.ConfigConstraint">
ConfigConstraint
...
<li>
<code>reloadAction</code> is set.
</li>
</ol>

<p>
If <code>reloadAction</code> is not set or the modified parameters are not listed in <code>dynamicParameters</code>,
dynamic reloading will not be triggered.
</p>
...
<p>
Configures whether the dynamic reload specified in <code>reloadAction</code> applies only to dynamic parameters or
to all parameters (including static parameters).
</p>
<ul>
<li>
...
<h3 id="apps.kubeblocks.io/v1beta1.AutoTrigger">
AutoTrigger
...
<h3 id="apps.kubeblocks.io/v1beta1.CfgFileFormat">
CfgFileFormat
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1beta1.ConfigConstraintPhase">
ConfigConstraintPhase
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1beta1.ConfigConstraintSpec">
ConfigConstraintSpec
...
<h3 id="apps.kubeblocks.io/v1beta1.ConfigConstraintStatus">
ConfigConstraintStatus
...
<h3 id="apps.kubeblocks.io/v1beta1.DownwardAPIChangeTriggeredAction">
DownwardAPIChangeTriggeredAction
...
For example, a command might be executed when the &lsquo;role&rsquo; label of the Pod is updated.
</p>
</div>
<table>
<thead>
...
The name should match the regex pattern <code>^[a-z0-9]([a-z0-9\.\-]&ast;[a-z0-9])?$</code>.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.DynamicParameterSelectedPolicy">
...
<h3 id="apps.kubeblocks.io/v1beta1.DynamicReloadType">
DynamicReloadType
(<code>string</code> alias)
</h3>
<div>
...
<h3 id="apps.kubeblocks.io/v1beta1.FileFormatConfig">
FileFormatConfig
...
<p>
The config file format. Valid values are <code>ini</code>, <code>xml</code>, <code>yaml</code>, <code>json</code>,
<code>hcl</code>, <code>dotenv</code>, <code>properties</code> and <code>toml</code>. Each format has its own characteristics and use cases.
</p>
<ul>
<li>
ini: is a text-based content with a structure and syntax comprising key–value pairs for properties, reference wiki: <a href="https://en.wikipedia.org/wiki/INI_file">https://en.wikipedia.org/wiki/INI_file</a>
</li>
<li>
xml: refers to wiki: <a href="https://en.wikipedia.org/wiki/XML">https://en.wikipedia.org/wiki/XML</a>
</li>
<li>
yaml: supports for complex data types and structures.
</li>
<li>
json: refers to wiki: <a href="https://en.wikipedia.org/wiki/JSON">https://en.wikipedia.org/wiki/JSON</a>
</li>
<li>
...
<li>
dotenv: is a plain text file with simple key–value pairs, reference wiki: <a href="https://en.wikipedia.org/wiki/Configuration_file#MS-DOS">https://en.wikipedia.org/wiki/Configuration_file#MS-DOS</a>
</li>
<li>
...
<li>
toml: refers to wiki: <a href="https://en.wikipedia.org/wiki/TOML">https://en.wikipedia.org/wiki/TOML</a>
</li>
<li>
...
<h3 id="apps.kubeblocks.io/v1beta1.FormatterAction">
FormatterAction
...
<p>
Holds options specific to the &lsquo;ini&rsquo; file format.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.IniConfig">
IniConfig
...
<p>
IniConfig holds options specific to the &lsquo;ini&rsquo; file format.
</p>
</div>
<table>
<thead>
...
<h3 id="apps.kubeblocks.io/v1beta1.ParametersSchema">
ParametersSchema
...
<p>
Specifies the top-level key in the &lsquo;configSchema.cue&rsquo; that organizes the validation rules for parameters.
...
<h3 id="apps.kubeblocks.io/v1beta1.ReloadAction">
ReloadAction
...
<p>
In some scenarios, only specific pods (e.g., primary replicas) need to undergo a dynamic reload.
The <code>reloadedPodSelector</code> allows you to specify label selectors to target the desired pods for the reload process.
</p>

<p>
If the <code>reloadedPodSelector</code> is not specified or is nil, all pods managed by the workload will be considered for the dynamic
reload.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.ScriptConfig">
ScriptConfig
...
If not specified, it defaults to the &ldquo;default&rdquo; namespace.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.ShellTrigger">
ShellTrigger
...
<p>
Determines the synchronization mode of parameter updates with &ldquo;config-manager&rdquo;.
</p>
<ul>
<li>
&lsquo;True&rsquo;: Executes reload actions synchronously, pausing until completion.
</li>
<li>
&lsquo;False&rsquo;: Executes reload actions asynchronously, without waiting for completion.
</li>
</ul>
...
<ul>
<li>
&lsquo;True&rsquo;: Processes all changes in one batch reload.
</li>
<li>
&lsquo;False&rsquo;: Processes each change individually.
</li>
</ul>

<p>
Defaults to &lsquo;False&rsquo; if unspecified.
</p>
...
<h3 id="apps.kubeblocks.io/v1beta1.SignalType">
SignalType
(<code>string</code> alias)
</h3>
...
<h3 id="apps.kubeblocks.io/v1beta1.TPLScriptTrigger">
TPLScriptTrigger
...
<p>
Determines whether parameter updates should be synchronized with the &ldquo;config-manager&rdquo;.
Specifies the controller&rsquo;s reload strategy:
</p>
<ul>
<li>
If set to &lsquo;True&rsquo;, the controller executes the reload action in synchronous mode,
pausing execution until the reload completes.
</li>
<li>
If set to &lsquo;False&rsquo;, the controller executes the reload action in asynchronous mode,
...
<h3 id="apps.kubeblocks.io/v1beta1.ToolConfig">
ToolConfig
...
<h3 id="apps.kubeblocks.io/v1beta1.ToolsSetup">
ToolsSetup
...
<p>
This example copies the &ldquo;/bin/ob-tools&rdquo; binary from the image to &ldquo;/kb_tools/obtools&rdquo;.
</p>
</div>
<table>
<thead>
...
<h3 id="apps.kubeblocks.io/v1beta1.UnixSignalTrigger">
UnixSignalTrigger
...
<h3 id="workloads.kubeblocks.io/v1.InstanceSet">
InstanceSet
...
<p>
Represents a label query over pods that should match the desired replica count indicated by the <code>replica</code> field.
It must match the labels defined in the pod template.
More info: <a href="https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors">https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors</a>
</p>
...
The final name of each PVC is generated by appending the pod&rsquo;s identifier to the name specified in volumeClaimTemplates[*].name.
</p>
...
<p>
The default policy is <code>OrderedReady</code>, where pods are created in increasing order and the controller waits until each pod is ready before
...
<p>
A list of roles defined in the system. Instanceset obtains role through pods&rsquo; role label <code>kubeblocks.io/role</code>.
</p>
...
<h3 id="workloads.kubeblocks.io/v1.ConditionType">
ConditionType
(<code>string</code> alias)
</h3>
<div>
...
and continue for &ldquo;MinReadySeconds&rdquo; seconds. Otherwise, it will be set to False.
</p>
...
<p>
InstanceFailure is added in an instance set when at least one of its instances(pods) is in a <code>Failed</code> phase.
</p>
...
<h3 id="workloads.kubeblocks.io/v1.ConfigTemplate">
ConfigTemplate
...
<h3 id="workloads.kubeblocks.io/v1.InstanceConfigStatus">
...
<h3 id="workloads.kubeblocks.io/v1.InstanceSetSpec">
InstanceSetSpec
...
<h3 id="workloads.kubeblocks.io/v1.InstanceSetStatus">
InstanceSetStatus
...
InstanceSet&rsquo;s generation, which is updated on mutation by the API Server.
</p>
...
<p>
Represents the latest available observations of an instanceset&rsquo;s current state.
Known .status.conditions.type are: &ldquo;InstanceFailure&rdquo;, &ldquo;InstanceReady&rdquo;
</p>
...
<h3 id="workloads.kubeblocks.io/v1.InstanceStatus">
InstanceStatus
...
<h3 id="workloads.kubeblocks.io/v1.InstanceTemplate">
InstanceTemplate
...
<h3 id="workloads.kubeblocks.io/v1.InstanceTemplateStatus">
InstanceTemplateStatus
...
<h3 id="workloads.kubeblocks.io/v1.MemberStatus">
MemberStatus
...
<h3 id="workloads.kubeblocks.io/v1.MemberUpdateStrategy">
MemberUpdateStrategy
(<code>string</code> alias)
</h3>
...
<h3 id="workloads.kubeblocks.io/v1.MembershipReconfiguration">
MembershipReconfiguration
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceSet">
InstanceSet
...
<h3 id="workloads.kubeblocks.io/v1alpha1.AccessMode">
AccessMode
(<code>string</code> alias)
</h3>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.Action">
Action
...
<h3 id="workloads.kubeblocks.io/v1alpha1.ConditionType">
ConditionType
(<code>string</code> alias)
</h3>
<div>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.Credential">
Credential
...
<p>
Defines the user&rsquo;s name for the credential.
...
<p>
Represents the user&rsquo;s password for the credential.
...
<h3 id="workloads.kubeblocks.io/v1alpha1.CredentialVar">
CredentialVar
...
<p>
Defines the source for the environment variable&rsquo;s value. This field is optional and cannot be used if the &lsquo;Value&rsquo; field is not empty.
</p>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceSetSpec">
InstanceSetSpec
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceSetStatus">
InstanceSetStatus
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceTemplate">
InstanceTemplate
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceTemplateStatus">
InstanceTemplateStatus
...
<h3 id="workloads.kubeblocks.io/v1alpha1.InstanceUpdateStrategy">
InstanceUpdateStrategy
...
<h3 id="workloads.kubeblocks.io/v1alpha1.MemberStatus">
MemberStatus
...
<h3 id="workloads.kubeblocks.io/v1alpha1.MemberUpdateStrategy">
MemberUpdateStrategy
(<code>string</code> alias)
</h3>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.MembershipReconfiguration">
MembershipReconfiguration
...
If the Image is not configured, the latest <a href="https://busybox.net/">BusyBox</a> image will be used.
</p>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.Ordinals">
Ordinals
...
<h3 id="workloads.kubeblocks.io/v1alpha1.PodUpdatePolicyType">
PodUpdatePolicyType
(<code>string</code> alias)
</h3>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.Range">
Range
...
<h3 id="workloads.kubeblocks.io/v1alpha1.ReplicaRole">
ReplicaRole
...
<h3 id="workloads.kubeblocks.io/v1alpha1.RoleProbe">
RoleProbe
...
<h3 id="workloads.kubeblocks.io/v1alpha1.RoleUpdateMechanism">
RoleUpdateMechanism
(<code>string</code> alias)
</h3>
...
<h3 id="workloads.kubeblocks.io/v1alpha1.SchedulingPolicy">
SchedulingPolicy
