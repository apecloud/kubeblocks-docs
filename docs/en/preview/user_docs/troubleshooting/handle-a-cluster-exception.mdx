...
By describing the ComponentDefinition, you can see following messsage:
...
### Cluster/Component stucks in `Updating` status
...
status:
  containerStatuses:
    ...
    name: c1
```
If the two fileds are not match, please check if there are two or more iamges share the same `IMAGG ID` but of different `IMAGE` tags.
