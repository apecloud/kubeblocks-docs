...
To try out KubeBlocks on your local host, you can [create a local Kubernetes cluster first](../references/prepare-a-local-k8s-cluster) and then follow the steps in this tutorial to install KubeBlocks.
...
- Make sure you have [kubectl](https://kubernetes.io/docs/tasks/tools/), [Helm](https://helm.sh/docs/intro/install/), or [kbcli](./../references/install-kbcli.mdx) installed.
...
1. Get the KubeBlocks version:
   ...
   * Option A - Get the latest stable version (e.g., v1.0.0):
...
2. Specify a version with `--version` and run the command below:
  ...
  :::note
   ...
   For example, you can install kbcli v1.0.0 with KubeBlocks v0.9.1, but using mismatched major versions, such as kbcli v1.0.0 with KubeBlocks v1.0.0, may lead to errors.
...
Please elete all clusters and backups before uninstalling KubeBlocks and kbcli.
...
```bash
helm list -n kb-system uninstall <kb-addon-name>
```
...
```
elease "kb-addon-etcd" uninstalled
These resources were kept due to the resource policy:
[ConfigMap] kafka27-configuration-tpl-1.0.0-alpha.0
[ComponentDefinition] kafka-combine-1.0.0-alpha.0
[ComponentDefinition] kafka-controller-1.0.0-alpha.0
[ComponentDefinition] kafka-exporter-1.0.0-alpha.0
[ComponentDefinition] kafka27-broker-1.0.0-alpha.0
[ComponentDefinition] kafka-broker-1.0.0-alpha.0
```
...
kubectl get parametersdefinitions.apps.kubeblocks.io
...
5. verify all kubeblocks resource are deleted
...
7. verify all kubeblocks resource are deleted
