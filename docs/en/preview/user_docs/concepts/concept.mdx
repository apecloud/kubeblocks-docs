...
Here is an example that creates a Redis Cluster with `clusterDef` and `topology`:
          ...
          spec:
            ...
            resources:
              requests:
                storage: 20Gi
```
If you have a sharp eye, you'll notice that by specifying `clusterDef` and `topology` in Cluster, you no longer need to specify `componentDef` for each Component.
...
![KubeBlocks API Layers](/img/docs/en/kubeblocks_api_layers.png)
