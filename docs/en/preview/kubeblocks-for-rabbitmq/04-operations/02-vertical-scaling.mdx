---
title: Vertical Scaling in a RabbitMQ ReplicationSet Cluster
description: Learn how to perform vertical scaling in a RabbitMQ ReplicationSet Cluster managed by KubeBlocks to optimize resource utilization and improve performance.
keywords: [KubeBlocks, RabbitMQ, Vertical Scaling, Kubernetes, Resources]
sidebar_position: 2
...
This guide demonstrates how to vertically scale a RabbitMQ ReplicationSet Cluster managed by KubeBlocks by adjusting compute resources (CPU and memory) while maintaining the same number of replicas.

Vertical scaling modifies compute resources (CPU/memory) for RabbitMQ instances while maintaining replica count. Key characteristics:
...
## Cleanup
To remove all created resources, delete the RabbitMQ ReplicationSet Cluster along with its namespace:
```bash
...
1. Deploy a RabbitMQ ReplicationSet Cluster managed by KubeBlocks.
...
Vertical scaling is a powerful tool for optimizing resource utilization and adapting to changing workload demands, ensuring your RabbitMQ ReplicationSet Cluster remains performant and resilient.