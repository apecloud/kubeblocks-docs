---
title: MongoDB ReplicationSet Cluster Lifecycle Management (Stop, Start, Restart)
description: Learn how to manage MongoDB ReplicationSet Cluster states in KubeBlocks including stopping, starting, and restarting operations to optimize resources.
...
# MongoDB ReplicationSet Cluster Lifecycle Management

This guide demonstrates how to manage a MongoDB ReplicationSet Cluster's operational state in **KubeBlocks**, including:
...
Stopping a MongoDB ReplicationSet Cluster in KubeBlocks will:
...
<Tabs>
  ...
  ```bash
  kubectl patch cluster mongo-cluster -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "mongodb", "stop": true}]}}'
  ```
...
Starting a stopped MongoDB ReplicationSet Cluster:
...
<Tabs>
  ...
  kubectl patch cluster mongo-cluster -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "mongodb", "stop": false}]}}'
  ```
...
1. Stop a MongoDB ReplicationSet Cluster to suspend operations while retaining persistent storage.
...
By managing the lifecycle of your MongoDB ReplicationSet Cluster, you can optimize resource utilization, reduce costs, and maintain flexibility in your Kubernetes environment. KubeBlocks provides a seamless way to perform these operations, ensuring high availability and minimal disruption.
