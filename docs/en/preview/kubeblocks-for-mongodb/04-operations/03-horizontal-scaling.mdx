...
<Tabs>
  ...
  ```bash
  kubectl patch cluster mongo-cluster -n demo --type=json -p='[{"op": "replace", "path": "/spec/componentSpecs/0/replicas", "value": 3}]'
  ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  spec:
    componentSpecs:
      - name: mongodb
        replicas: 1 # decrease replcias to scale-out
  ```
