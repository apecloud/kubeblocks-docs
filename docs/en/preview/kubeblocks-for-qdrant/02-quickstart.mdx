...
## Deploy a Qdrant Replication Cluster

Deploy a basic Qdrant Cluster with default settings:

```bash
kubectl apply -f https://github.com/apecloud/kubeblocks-addons/blob/main/examples/qdrant/cluster.yaml
```

This creates:
- A Qdrant Cluster with two components, Qdrant(2 replicas) and Qdrant Sentinel(3 replicas).
...
When deploying a Qdrant Cluster with 1 primary replica and 2 secondary replicas:
