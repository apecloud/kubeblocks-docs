---
title: Qdrant ReplicationSet Cluster Lifecycle Management (Stop, Start, Restart)
description: Learn how to manage Qdrant ReplicationSet Cluster states in KubeBlocks including stopping, starting, and restarting operations to optimize resources.
...
# Qdrant ReplicationSet Cluster Lifecycle Management

This guide demonstrates how to manage a Qdrant ReplicationSet Cluster's operational state in **KubeBlocks**, including:
...
Stopping a Qdrant ReplicationSet Cluster in KubeBlocks will:
...
<Tabs>
  ...
  ```bash
  kubectl patch cluster qdrant-cluster -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "qdrant", "stop": true}]}}'
  ```
...
Starting a stopped Qdrant ReplicationSet Cluster:
...
<Tabs>
  ...
  kubectl patch cluster qdrant-cluster -n demo \
  --type=merge \
  -p='{"spec": {"componentSpecs": [{"name": "qdrant", "stop": false}]}}'
  ```
...
1. Stop a Qdrant ReplicationSet Cluster to suspend operations while retaining persistent storage.
...
By managing the lifecycle of your Qdrant ReplicationSet Cluster, you can optimize resource utilization, reduce costs, and maintain flexibility in your Kubernetes environment. KubeBlocks provides a seamless way to perform these operations, ensuring high availability and minimal disruption.
