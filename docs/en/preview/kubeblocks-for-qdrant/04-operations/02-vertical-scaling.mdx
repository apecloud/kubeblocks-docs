---
title: Vertical Scaling in a Qdrant ReplicationSet Cluster
description: Learn how to perform vertical scaling in a Qdrant ReplicationSet Cluster managed by KubeBlocks to optimize resource utilization and improve performance.
...
# Vertical Scaling for Qdrant Replication Clusters with KubeBlocks

This guide demonstrates how to vertically scale a Qdrant ReplicationSet Cluster managed by KubeBlocks by adjusting compute resources (CPU and memory) while maintaining the same number of replicas.
...
## Cleanup
To remove all created resources, delete the Qdrant ReplicationSet Cluster along with its namespace:
```bash
...
1. Deploy a Qdrant ReplicationSet Cluster managed by KubeBlocks.
...
Vertical scaling is a powerful tool for optimizing resource utilization and adapting to changing workload demands, ensuring your Qdrant ReplicationSet Cluster remains performant and resilient.