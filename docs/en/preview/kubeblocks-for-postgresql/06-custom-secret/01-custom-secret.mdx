...
In KubeBlocks PostgreSQL Addon, a list of system accounts are defined. And only those accounts can be `customized` with a new secret.

To get the of of accounts:
```bash
kubectl get cmpd postgresql-16-1.0.0-alpha.0 -oyaml | yq '.spec.systemAccounts[].name'
```
