...
description: How to recover a PostgreSQL replica in a replication cluster managed by KubeBlocks using in-place and non-in-place repair methods.
keywords: [KubeBlocks, PostgreSQL, Replica Recovery, In-Place Repair, Non-In-Place Repair]
sidebar_position: 11
sidebar_label: Recovering PostgreSQL Replica
---

# Recovering PostgreSQL Replica in KubeBlocks

This guide demonstrates how to perform the following tasks in a PostgreSQL replication cluster managed by KubeBlocks:
...
## Connect to the Primary PostgreSQL Cluster and Write Mock Data
...
**Expected Workflow**:
1. The old Pod (e.g. 'pg-cluster-postgresql-0') is terminated and created
2. PVC
2. The new Pod has all data as the primary node.
...
persistentvolumeclaim/data-pg-cluster-postgresql-3   Bound    pvc-yyy   20Gi       RWO            <SC>          <unset>                 14m
```
...
**Expected Workflow**:
1. A new Pod (e.g. 'pg-cluster-postgresql-2') is created first
2. The old Pod (e.g. 'pg-cluster-postgresql-0') is terminated when the new replica is running.
3. The new Pod has all data as the primary node.
