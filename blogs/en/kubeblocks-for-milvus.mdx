---
slug: kubeblocks-for-milvus-revealed
title: KubeBlocks for Milvus Revealed
description: This blog introduces the Milvus vector database and its deployment implementation on KubeBlocks, covering standalone and cluster deployment modes using modular Addons like etcd, Kafka, and MinIO.
date: 2025-08-12
authors:
  name: Cheng Jiacheng
...
# KubeBlocks for Milvus Revealed
...
KubeBlocks is a database-neutral control plane running on Kubernetes. Through the abstraction of Addons, KubeBlocks provides a unified interface supporting any type of database, covering various aspects such as startup scripts, lifecycle hooks, and configuration management. The KubeBlocks Milvus Addon not only manages the Milvus lifecycle and operations but also provides better management of the external components that Milvus depends on. Through the database operations interface provided by KubeBlocks, users can have a unified experience when operating various databases.
